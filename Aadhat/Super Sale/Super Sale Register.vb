...
Public Class Super_Sale_Register
    ...
    Public Sub retrive(Optional ByVal condtion As String = "")
        ...
        Dim RecordCount As Integer = 0
        dt = clsFun.ExecDataTable("Select * FROM Vouchers v INNER JOIN Transaction2 t ON v.id = t.VoucherID INNER JOIN Accounts a1 ON a1.ID = v.sallerID INNER JOIN Accounts a ON a.ID = t.AccountID INNER JOIN Items i ON i.ID = t.ItemID Where v.EntryDate Between '" & CDate(txtFromDate.Text).ToString("yyyy-MM-dd") & "' And '" & CDate(txtToDate.Text).ToString("yyyy-MM-dd") & "' and v.transtype='" & Me.Text & "' " & condtion & "  order by  v.InvoiceID,v.EntryDate ")
        ...
        For Each row As DataGridViewRow In filteredRows
            With row
                If .Cells(3).Value.trim() <> String.Empty Then
                    ...
                    GlobalData.PdfName = .Cells(4).Value.Replace("/", "") & "-" & .Cells(2).Value & ".pdf"
    ...
    Private Sub ShowWhatsappContacts(Optional ByVal condtion As String = "")
        ...
        Dim i As Integer
        Dim count As Integer = 0
        Dim SSql As String = "Select * From Vouchers Where TransType='Super Sale' and EntryDate Between '" & CDate(txtFromDate.Text).ToString("yyyy-MM-dd") & "' and '" & CDate(txtToDate.Text).ToString("yyyy-MM-dd") & "' Order by Cast(BIllNo as Integer)"
    ...
    Private Sub Button6_Click(sender As Object, e As EventArgs) Handles Button6.Click
        If cbType.Text = MobileSendingMethod Then
            ...
            Exit Sub
        End If
        If cbType.Text = "WhatsApp Official API" Then
            SendOfficialSuperSaleRegisterData()
            Exit Sub
        End If
        Dim WhatsappFile As String = Application.StartupPath & "\Whatsapp\Easy Whatsapp.exe"
        If System.IO.File.Exists(WhatsappFile) = False Then
    ...
    Private Sub EnsureMobileWhatsappControls()
        ...
        Label13.Visible = (cbType.Text <> MobileSendingMethod) : txtSearchAccount.Visible = (cbType.Text <> MobileSendingMethod)
    End Sub
        ...
        For Each row As DataGridViewRow In DgWhatsapp.Rows
            With row
                ...
                If fileUrl.StartsWith("http", StringComparison.OrdinalIgnoreCase) = False Then
                    ...
                    Continue For
                End If
                Dim apiResult As String = PhoneMSg.SendPhoneMsg(mobile, cbSim.Text, "SUPER SALE VIEW : " & fileUrl.Trim())
                .Cells(5).Value = apiResult
            End With
        Next
        UpdateProgressBarVisibility(False)
        MsgBox("Super Sale Register sent on msgz.in server", vbInformation, "Sended")
    End Sub

    Private Sub SendOfficialSuperSaleRegisterData()
        Dim directoryName As String = Application.StartupPath & "\Pdfs"
        ...
        Next
        UpdateProgressBarVisibility(False)
        MsgBox("Super Sale Register sent via Official API", vbInformation, "Official API")
    End Sub
