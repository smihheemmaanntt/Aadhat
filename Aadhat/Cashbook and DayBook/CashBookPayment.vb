Public Class CashBookPayment
    ...
    Private Sub RetriveOld()
        ...
        Try
            ...
            ssql = "Select Entrydate from Ledger where DC ='D' " & IIf(cbAccountName.SelectedValue > 0, "and AccountID=" & Val(cbAccountName.SelectedValue) & "", "") & " and EntryDate Between '" & CDate(txtFromDate.Text).ToString("yyyy-MM-dd") & "' And '" & CDate(txttoDate.Text).ToString("yyyy-MM-dd") & "'   union " & _
                " Select Entrydate from Ledger where Dc='C' " & IIf(cbAccountName.SelectedValue > 0, "and AccountID=" & Val(cbAccountName.SelectedValue) & "", "") & " and EntryDate Between '" & CDate(txtFromDate.Text).ToString("yyyy-MM-dd") & "' And '" & CDate(txttoDate.Text).ToString("yyyy-MM-dd") & "' order by Entrydate "
            ...
            If dt.Rows.Count > 0 Then
                For i = 0 To dt.Rows.Count - 1
                    ...
                    ssql = "Select Entrydate, TransType,AccountName,Remark,sum(Amount) as Dr,'0' as Cr,Narration from Ledger where DC ='D' " & IIf(cbAccountName.SelectedValue > 0, "and AccountID=" & Val(cbAccountName.SelectedValue) & "", "") & " and EntryDate = '" & CDate(dt.Rows(i)("Entrydate")).ToString("yyyy-MM-dd") & "'  Group by Transtype   union all" & _
                    ...
                    For j = 0 To tmpDt.Rows.Count - 1
                        ...
                        With dg1.Rows(lastval)
                            If tmpDt.Rows(j)("Dr").ToString() <> "0" Then
                                dg1.Rows(lastval).Cells(1).Style.Alignment = DataGridViewContentAlignment.MiddleLeft
                                .Cells(1).Value = tmpDt.Rows(j)("TransType").ToString() & " All"
    ...
    Private Sub RetriveNew()
        ...
        Try
            ...
            ssql = "Select Entrydate from Ledger where DC ='D' " & IIf(cbAccountName.SelectedValue > 0, "and AccountID in('" & Val(cbAccountName.SelectedValue) & "','17')", "") & " and EntryDate Between '" & CDate(txtFromDate.Text).ToString("yyyy-MM-dd") & "' And '" & CDate(txttoDate.Text).ToString("yyyy-MM-dd") & "'   union " & _
                         " Select Entrydate from Ledger where Dc='C' " & IIf(cbAccountName.SelectedValue > 0, "and AccountID in('" & Val(cbAccountName.SelectedValue) & "','17')", "") & " and EntryDate Between '" & CDate(txtFromDate.Text).ToString("yyyy-MM-dd") & "' And '" & CDate(txttoDate.Text).ToString("yyyy-MM-dd") & "' order by Entrydate "
            opbal = clsFun.ExecScalarStr(" Select sum(OpBal) FROM Accounts WHERE ID in (" & cbAccountName.SelectedValue & ",17)")
            ...
            If dt.Rows.Count > 0 Then
                For i = 0 To dt.Rows.Count - 1
                    ...
                    lastval = lastval + 1
                    dg1.Rows.Add()
                    Dim tmpamtdr As String = clsFun.ExecScalarStr("Select sum(Amount) as tot from Ledger where Dc='D' and AccountID in('" & Val(cbAccountName.SelectedValue) & "','17') and EntryDate < '" & CDate(dt.Rows(i)("Entrydate")).ToString("yyyy-MM-dd") & "'")
                    Dim tmpamtcr As String = clsFun.ExecScalarStr("Select sum(Amount) as tot from Ledger where Dc='C' and AccountID in('" & Val(cbAccountName.SelectedValue) & "','17') and EntryDate < '" & CDate(dt.Rows(i)("Entrydate")).ToString("yyyy-MM-dd") & "'")
                    ''Dim tmpamt As String = clsFun.ExecScalarStr("Select sum(Amount) as tot from Ledger where accountID=" & Val(cbAccountName.SelectedValue) & " and EntryDate < '" & CDate(dt.Rows(i)("Entrydate")).ToString("yyyy-MM-dd") & "'")
                    If drcr = "Dr" Then
                        tmpamtdr = Val(opbal) + Val(tmpamtdr)
                    Else
                    ...
                    '************For Opening Balance"***************************************************************************
                    If i = 0 Then
                        Dim cnt As Integer = clsFun.ExecScalarInt("Select count(*) from LEdger where  AccountID in('" & Val(cbAccountName.SelectedValue) & "','17') and  EntryDate < '" & CDate(dt.Rows(i)("Entrydate")).ToString("yyyy-MM-dd") & "'")
                        If cnt = 0 Then
                            If drcr = "Dr" Then
                                dg1.Rows(lastval).Cells(1).Style.Alignment = DataGridViewContentAlignment.BottomLeft
                                dg1.Rows(lastval).Cells(1).Style.ForeColor = Color.Blue
                    ...
                    '****************************Grid Fill Based on Dates********************************************************
                    ssql = "Select Entrydate, TransType,AccountName,Remark,sum(Amount) as Dr,'0' as Cr,Narration from Ledger where DC ='D' and TransType<>'Payment' " & IIf(cbAccountName.SelectedValue > 0, "and AccountID in('" & Val(cbAccountName.SelectedValue) & "','17')", "") & " and EntryDate = '" & CDate(dt.Rows(i)("Entrydate")).ToString("yyyy-MM-dd") & "'  Group by Transtype   union all" & _
                            " Select Entrydate,  TransType,AccountName,Remark,'0' as Dr,Amount as Cr ,Narration from Ledger where Dc='C' " & IIf(cbAccountName.SelectedValue > 0, "and AccountID=" & Val(cbAccountName.SelectedValue) & "", "") & " and EntryDate = '" & CDate(dt.Rows(i)("Entrydate")).ToString("yyyy-MM-dd") & "'    "
    ...
    Private Sub PrintRecord()
        ...
        For Each row As DataGridViewRow In dg1.Rows
            With row
                sql = sql & "insert into Printing(D1,D2,M1, P1, P2,P3, P4, P5, P6,P7) values('" & txtFromDate.Text & "'," & _
                    "'" & txttoDate.Text & "','" & cbAccountName.Text & "'," & _
    ...
    Private Sub CheckBox1_CheckedChanged(sender As Object, e As EventArgs) Handles ckDiscountAlso.CheckedChanged
