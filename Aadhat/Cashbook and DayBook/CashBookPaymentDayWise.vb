Public Class CashBookPaymentDayWise
    ...
    End Sub
    Private Sub mskFromDate_KeyDown(sender As Object, e As KeyEventArgs) Handles txtEntryDate.KeyDown, cbAccountName.KeyDown
    ...
    Private Sub Cash_Bank_Book_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        txtEntryDate.Text = IIf(mindate <> "", mindate, Date.Today.ToString("dd-MM-yyyy"))
        txtEntryDate.Text = IIf(maxdate <> "", maxdate, Date.Today.ToString("dd-MM-yyyy"))
    ...
    Private Sub RetriveTemp()
        ...
        Sql = "Select sum(Amount) as Total From Ledger Where EntryDate='" & CDate(txtEntryDate.Text).ToString("yyyy-MM-dd") & "'  and TransType='Receipt' and DC='D' And PartyID in(Select ID From Accounts Where GroupID=32)" & _
        ...
        For i As Integer = 0 To dt.Rows.Count - 1
            ...
            With DgLibilities.Rows(lastval)
                ...
                .Cells(2).Value = Val(dt.Rows(i)("Total").ToString()) + Val(clsFun.ExecScalarStr("Select Sum(Amount) as Total From Ledger Where EntryDate='" & CDate(txtEntryDate.Text).ToString("yyyy-MM-dd") & "' and AccountID=17 and DC='D' and TransType='Receipt' and PartyID=" & Val(cbAccountName.SelectedValue) & ";"))
                lastval = lastval + 1
            End With
        Next
        Sql = String.Empty
        Sql = "Select TransType,sum(Amount) as Total From Ledger Where EntryDate='" & CDate(txtEntryDate.Text).ToString("yyyy-MM-dd") & "'  and TransType<>'Receipt' and DC='D' and AccountID=" & Val(cbAccountName.SelectedValue) & " Group by TransType;"
        dt = clsFun.ExecDataTable(Sql)
        For i As Integer = 0 To dt.Rows.Count - 1
            DgLibilities.Rows.Add()
            With DgLibilities.Rows(lastval)
        ...
        Next
        Sql = String.Empty
        Sql = "Select PartyName,Amount,TransType as Total From Ledger Where EntryDate='" & CDate(txtEntryDate.Text).ToString("yyyy-MM-dd") & "' and TransType in ('Receipt','Group Receipt') and DC='D' " & _
        ...
        Sql = "Select PartyName,TransType,Amount From Ledger Where DC='C' and TransType in ('Payment','Cash Deposit','Group Payment') and EntryDate='" & CDate(txtEntryDate.Text).ToString("yyyy-MM-dd") & "' and AccountID=" & Val(cbAccountName.SelectedValue) & ""
        dt = clsFun.ExecDataTable(Sql)
        For i As Integer = 0 To dt.Rows.Count - 1
            dgAssests.Rows.Add()
    ...
    Private Sub Retrive()
        ...
        Sql = "Select ID,Accountname,Area,Opbal,DC,OtherName,Mobile1, " & _
           ...
           " +(Select ifnull(Round(Sum(Amount),2),0) From Ledger Where AccountID=Accounts.ID and DC='D' and Ledger.Entrydate <'" & CDate(txtEntryDate.Text).ToString("yyyy-MM-dd") & "'))  end),2) as  Restbal from Accounts Where RestBal<>0 and ID=" & Val(cbAccountName.SelectedValue) & "  Order by upper(AccountName) ;"
        dt = clsFun.ExecDataTable(Sql)
        ...
        For j As Integer = 0 To IIf(DgLibilities.Rows.Count > dgAssests.Rows.Count, DgLibilities.Rows.Count, dgAssests.Rows.Count) - 1
            ...
            lastval = lastval + 1
        Next
        Sql = "Select Sum(Amount) as Total From Ledger Where EntryDate='" & CDate(txtEntryDate.Text).ToString("yyyy-MM-dd") & "' and AccountID=17 and DC='D' and TransType='Receipt' and PartyID=" & Val(cbAccountName.SelectedValue) & ";"
        dt = clsFun.ExecDataTable(Sql)
        If dt.Rows.Count > 0 Then
            For i = 0 To dt.Rows.Count - 1
        ...
        For i As Integer = 0 To maxRowCount - 1
            ...
            For LastCount = 0 To IIf(i = (maxRowCount - 1), Val(TotalRecord - 1), 99)
                With dg1.Rows(LastRecord)
                    ...
                    "'" & txtEntryDate.Text & "','" & cbAccountName.Text & "'," & _
    ...
    Private Sub PrintRecord1()
        ...
        For Each row As DataGridViewRow In dg1.Rows
            With row
                sql = sql & "insert into Printing(D1,D2,M1, P1, P2,P3, P4, P5, P6,P7) values('" & txtEntryDate.Text & "'," & _
                    "'" & txtEntryDate.Text & "','" & cbAccountName.Text & "'," & _
                    "'" & .Cells(1).Value & "','" & .Cells(2).Value & "','" & .Cells(3).Value & "','" & .Cells(4).Value & "'," & _
                    "'" & .Cells(5).Value & "','" & .Cells(6).Value & "','" & .Cells(7).Value & "');"
            End With
