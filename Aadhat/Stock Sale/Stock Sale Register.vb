Public Class Stock_Sale_Register
    ...
    Private Sub rowColums()
        ...
        dg1.Columns(4).Name = "Seller" : dg1.Columns(4).Width = 130
        dg1.Columns(5).Name = "GR" : dg1.Columns(5).Width = 50
        dg1.Columns(6).Name = "Item" : dg1.Columns(6).Width = 100
        dg1.Columns(7).Name = "Lot" : dg1.Columns(7).Width = 60
        dg1.Columns(8).Name = "Customer" : dg1.Columns(8).Width = 140
        dg1.Columns(9).Name = "Nug" : dg1.Columns(9).Width = 60
        dg1.Columns(10).Name = "Kg" : dg1.Columns(10).Width = 60
        dg1.Columns(11).Name = "Rate" : dg1.Columns(11).Width = 60
        dg1.Columns(12).Name = "Per" : dg1.Columns(12).Width = 50
        dg1.Columns(13).Name = "Basic" : dg1.Columns(13).Width = 80
        dg1.Columns(14).Name = "Charges" : dg1.Columns(14).Width = 80
        dg1.Columns(15).Name = "R.Off" : dg1.Columns(15).Width = 60
        dg1.Columns(16).Name = "Total" : dg1.Columns(16).Width = 100
    End Sub
    Sub calc()
        ...
        For i = 0 To dg1.Rows.Count - 1
            txtTotNug.Text = Format(Val(txtTotNug.Text) + Val(dg1.Rows(i).Cells(9).Value), "0.00")
            txtTotweight.Text = Format(Val(txtTotweight.Text) + Val(dg1.Rows(i).Cells(10).Value), "0.00")
            txtTotBasic.Text = Format(Val(txtTotBasic.Text) + Val(dg1.Rows(i).Cells(13).Value), "0.00")
            txtTotCharge.Text = Format(Val(txtTotCharge.Text) + Val(dg1.Rows(i).Cells(14).Value), "0.00")
            txtTotROff.Text = Format(Val(txtTotROff.Text) + Val(dg1.Rows(i).Cells(15).Value), "0.00")
            TxtGrandTotal.Text = Format(Val(TxtGrandTotal.Text) + Val(dg1.Rows(i).Cells(16).Value), "0.00")
        Next
    End Sub
    ...
    Private Sub retrive(Optional ByVal Primary As String = "", Optional ByVal Secondary As String = "")
        ...
        dt = clsFun.ExecDataTable("Select * FROM Stock_Sale_Report Where EntryDate Between '" & CDate(mskFromDate.Text).ToString("yyyy-MM-dd") & "' And '" & CDate(MsktoDate.Text).ToString("yyyy-MM-dd") & "' and transtype='" & Me.Text & "' " & Primary & "" & Secondary & "  order by EntryDate,cast(BillNo as Integer),Voucherid ")
        'dt = clsFun.ExecDataTable("Select * FROM Transaction2 WHERE EntryDate Betwjeen '" & CDate(mskFromDate.Text).ToString("yyyy-MM-dd") & "' And '" & CDate(MsktoDate.Text).ToString("yyyy-MM-dd") & "' and transtype='" & Me.Text & "' order by EntryDate")
                ...
                For i = 0 To dt.Rows.Count - 1
                    ...
                    With dg1.Rows(i)
                        ...
                        Else
                            ...
                            RecordCount = RecordCount + 1
                        End If
                        .Cells(5).Value = dt.Rows(i)("PBIll").ToString()
                        .Cells(6).Value = dt.Rows(i)("ItemName").ToString()
                        .Cells(7).Value = dt.Rows(i)("Lot").ToString()
                        .Cells(8).Value = dt.Rows(i)("AccountName").ToString()
                        .Cells(9).Value = Format(Val(dt.Rows(i)("Nug").ToString()), "0.00")
                        .Cells(10).Value = Format(Val(dt.Rows(i)("Weight").ToString()), "0.00")
                        .Cells(11).Value = Format(Val(dt.Rows(i)("Rate").ToString()), "0.00")
                        .Cells(12).Value = dt.Rows(i)("Per").ToString()
                        .Cells(13).Value = Format(Val(dt.Rows(i)("Amount").ToString()), "0.00")
                        .Cells(14).Value = Format(Val(dt.Rows(i)("Charges").ToString()), "0.00")
                        .Cells(16).Value = Format(Val(dt.Rows(i)("TotalAmount").ToString()), "0.00")
                        .Cells(15).Value = Format(Val(.Cells(16).Value) - Val(Val(.Cells(14).Value) + Val(.Cells(13).Value)), "0.00")
                    End With
    ...
    Private Sub PrintRecord()
        ...
        For i As Integer = 0 To maxRowCount - 1
            ...
            For LastCount = 0 To IIf(i = (maxRowCount - 1), Val(TotalRecord - 1), 99)
                With dg1.Rows(LastRecord)
                    FastQuery = FastQuery & IIf(FastQuery <> "", " UNION ALL SELECT ", " SELECT ") & "'" & mskFromDate.Text & "','" & MsktoDate.Text & "'," & _
                        ...
                        "'" & txtTotBasic.Text & "','" & txtTotCharge.Text & "','" & TxtGrandTotal.Text & "','" & .Cells("GR").Value & "'"
                    '        End With
                End With
            ...
            Try
                ...
                sQL = "insert into Printing(D1,D2,M1,M2, P1, P2,P3, P4, P5, P6,P7,P8,P9,P10,T1,T2,T3,T4,T5,P11) " & FastQuery & ""
    ...
    Private Sub txtSearchPrimary_KeyPress(sender As Object, e As KeyPressEventArgs) Handles txtSearchPrimary.KeyPress, txtSearchSecondary.KeyPress
        If e.KeyChar = "'"c Then
            e.Handled = True
        End If
    End Sub
