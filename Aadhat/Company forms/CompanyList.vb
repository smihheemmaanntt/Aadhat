    ...
    Private Sub UpdateView()
        ...
        If clsFun.CheckIfColumnExists("BillPrints2", "GrossWeight") = False Then
            ...
            sql = " DROP VIEW IF EXISTS BillPrints2;" & _
                  ...
                  " SELECT Transaction2.EntryDate, Items.ItemName, Accounts.AccountName, sum(Transaction2.Nug) AS Nug, " & _
                  ...
                  " Transaction2.MPer, sum(Transaction2.MAmt) AS MAmt, Transaction2.RdfPer, sum(Transaction2.RdfAmt) AS RdfAmt, Transaction2.Tare, sum(Transaction2.Tareamt) AS TareAmt, " & _
                  " Transaction2.Labour, sum(Transaction2.LabourAmt) AS LabourAmt, Transaction2.MaintainCrate, Transaction2.Cratemarka, Transaction2.CrateQty," & _
                  " Items.OtherName, Accounts.Othername AS AccountNameOther, Accounts.Mobile1 AS MobileNo1, Accounts.Mobile2 AS MobileNo2, Accounts.LFNo AS LFNo, Accounts.Area AS Area, " & _
                  " Accounts.City, Transaction2.TransType, Transaction2.AccountID, ifnull(Transaction2.OnWeight,'') as OnWeight, ifnull(Transaction2.Lot,'') as lot, ifnull(Transaction2.Cut,'') as Cut, ifnull(Transaction2.GrossWeight,'') AS GrossWeight" & _
                  " FROM (Transaction2 INNER JOIN Accounts ON Transaction2.AccountID = Accounts.ID) INNER JOIN Items ON Transaction2.ItemID = Items.ID" & _
