Public Class CustomerWiseSaleSummary
    ...
    Private Sub rowColums()
        ...
        dg1.Columns(1).Name = "Customer" : dg1.Columns(1).Width = 150
        dg1.Columns(2).Name = "Nug" : dg1.Columns(2).Width = 100
        dg1.Columns(3).Name = "Weight" : dg1.Columns(3).Width = 100
        dg1.Columns(4).Name = "Basic" : dg1.Columns(4).Width = 100
        dg1.Columns(5).Name = "Comm" : dg1.Columns(5).Width = 100
        dg1.Columns(6).Name = "M Tax" : dg1.Columns(6).Width = 100
        dg1.Columns(7).Name = "Rdf" : dg1.Columns(7).Width = 100
        dg1.Columns(8).Name = "Tara" : dg1.Columns(8).Width = 100
        dg1.Columns(9).Name = "Labour" : dg1.Columns(9).Width = 100
        dg1.Columns(10).Name = "R.off" : dg1.Columns(10).Width = 100
            ...
            If dt.Rows.Count > 0 Then
                ...
                For i = 0 To dt.Rows.Count - 1
                    ...
                    With dg1.Rows(i)
                        ...
                        .Cells(5).Value = Format(Val(dt.Rows(i)("CommAmt").ToString()), "0.00")
                        .Cells(6).Value = Format(Val(dt.Rows(i)("MAmt").ToString()), "0.00")
                        .Cells(7).Value = Format(Val(dt.Rows(i)("RDF").ToString()), "0.00")
                        .Cells(8).Value = Format(Val(dt.Rows(i)("Tare").ToString()), "0.00")
                        .Cells(9).Value = Format(Val(dt.Rows(i)("Labour").ToString()), "0.00")
                        .Cells(11).Value = Format(Val(dt.Rows(i)("TotalAmount").ToString()), "0.00")
                        .Cells(10).Value = Format(Val(.Cells(11).Value) - Val(Val(dt.Rows(i)("Charges").ToString() + Val(.Cells(4).Value))), "0.00")
                    End With
                Next
            End If
            dt.Dispose()
    ...
    Sub calc()
        ...
        txtCommission.Text = Format(0, "0.00") : txtCommission.Text = Format(0, "0.00")
        txtMTax.Text = Format(0, "0.00") : txtRdfAmt.Text = Format(0, "0.00")
        txtTare.Text = Format(0, "0.00") : txtLabour.Text = Format(0, "0.00")
        txtRoff.Text = Format(0, "0.00") : TxtGrandTotal.Text = Format(0, "0.00")
        txtTotalbasic.Text = Format(0, "0.00")
        Dim i As Integer
        For i = 0 To dg1.Rows.Count - 1
            ...
            txtCommission.Text = Format(Val(txtCommission.Text) + Val(dg1.Rows(i).Cells(5).Value), "0.00")
            txtMTax.Text = Format(Val(txtMTax.Text) + Val(dg1.Rows(i).Cells(6).Value), "0.00")
            txtRdfAmt.Text = Format(Val(txtRdfAmt.Text) + Val(dg1.Rows(i).Cells(7).Value), "0.00")
            txtTare.Text = Format(Val(txtTare.Text) + Val(dg1.Rows(i).Cells(8).Value), "0.00")
            txtLabour.Text = Format(Val(txtLabour.Text) + Val(dg1.Rows(i).Cells(9).Value), "0.00")
            txtRoff.Text = Format(Val(txtRoff.Text) + Val(dg1.Rows(i).Cells(10).Value), "0.00")
    ...
    Private Sub printRecord()
        ...
        For Each row As DataGridViewRow In dg1.Rows
            With row
                sql = sql & "insert into Printing(D1,D2,P1,P2, P3, P4,P5, P6, P7, P8,P9,P10,P11,P12,P13,P14,P15,P16,P17,P18,P19,P20,P21) values('" & mskFromDate.Text & "','" & MsktoDate.Text & "'," & _
                    "'" & .Cells(1).Value & "','" & .Cells(2).Value & "','" & .Cells(3).Value & "','" & .Cells(4).Value & "','" & .Cells(5).Value & "'," & _
                    "'" & Val(.Cells(6).Value) & "','" & Val(.Cells(7).Value) & "'," & Val(.Cells(8).Value) & ",'" & .Cells(9).Value & "'," & _
                    "'" & Val(.Cells(10).Value) & "'," & Val(.Cells(11).Value) & ",'" & Val(txtTotNug.Text) & "','" & Val(txtTotweight.Text) & "','" & Val(txtTotalbasic.Text) & "','" & Val(txtCommission.Text) & "'," & _
                    "'" & Val(txtMTax.Text) & "','" & Val(txtRdfAmt.Text) & "','" & Val(txtTare.Text) & "','" & Val(txtLabour.Text) & "','" & Val(txtRoff.Text) & "','" & Val(TxtGrandTotal.Text) & "');"
            End With
        Next
        Try
    ...
    Private Sub BtnPrint_Click(sender As Object, e As EventArgs) Handles BtnPrint.Click
        printRecord()
        Report_Viewer.printReport("\Reports\CustomerSalesReport.rpt")
