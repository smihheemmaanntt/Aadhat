...
Public Class ClsFunserver
    Public Shared ConString As String = "Data Source=" & Application.StartupPath & "\ServerDb\ServerDB.db;Version=3;New=True;Compress=True;synchronous=ON;"
    ...
    Public Shared Function Ledger(ByVal Opt As Integer, ByVal vchid As Integer, ByVal entryDt As String, ByVal TransType As String, _
                                  ByVal Acid As Integer, ByVal AcName As String, ByVal Amt As Double, ByVal Dc As String, ByVal ServerTag As Integer, _
                                  ByVal OrgID As Integer, Optional ByVal Remarks As String = "", Optional ByVal Narration As String = "", _
                                  Optional ByVal RemarkHindi As String = "") As Integer
        Ledger = 0
        Dim sSql As String = String.Empty
        If Opt = 0 Then
            sSql = "Insert into Ledger(VourchersID,EntryDate,TransType,AccountID,AccountName,Amount,DC,ServerTag,ORGID,Remark,Narration,RemarkHindi)" & _
                " values(" & vchid & ",'" & entryDt & "','" & TransType & "'," & Acid & ",'" & AcName & "'," & Amt & ",'" & Dc & "', " & _
                "'" & Val(ServerTag) & "','" & (OrgID) & "','" & Remarks & "','" & Narration & "','" & RemarkHindi & "')"
    ...
    Public Shared Function FastLedger(FastQuery As String) As Integer
        Dim sSql As String = String.Empty
        sSql = "Insert into Ledger(VourchersID,EntryDate,TransType,AccountID,AccountName,Amount,DC,ServerTag,OrgID,Remark,Narration,RemarkHindi) " & FastQuery & ";"
        FastQuery = ExecNonQuery(sSql, True)
        Return FastLedger
    End Function
    Public Shared Function CrateLedger(ByVal Opt As Integer, ByVal vchid As Integer, ByVal Slip As Integer, ByVal entryDt As String, _
                                       ByVal TransType As String, ByVal Acid As Integer, ByVal AcName As String, ByVal CrateType As String, _
                                       ByVal CrateID As Integer, ByVal MarkaName As String, ByVal qty As Integer, ByVal Remark As String, _
                                       ByVal Rate As String, ByVal Amount As String, ByVal CashPaid As String, ByVal ServerTag As Integer, ByVal OrgID As Integer) As Integer
        CrateLedger = 0
        Dim sSql As String = String.Empty
        If Opt = 0 Then
            sSql = "Insert into CrateVoucher(VoucherID,SlipNo, EntryDate,TransType,AccountID,AccountName,CrateType,CrateID,CrateName, Qty,Remark,Rate,Amount,CashPaid,ServerTag,OrgID)" & _
                " values(" & vchid & "," & Slip & ",'" & entryDt & "','" & TransType & "'," & Acid & ",'" & AcName & "','" & CrateType & "'," & CrateID & ",'" & MarkaName & "'," & qty & ", " & _
                "'" & Remark & "','" & Rate & "','" & Amount & "','" & CashPaid & "','" & Val(ServerTag) & "','" & (OrgID) & "')"
            CrateLedger = ExecNonQuery(sSql, True)
            'ElseIf Opt = 1 Then
            '    sSql = "Update Ledger SET EntryDate='" & entryDt & "',TransType='" & TransType & "',AccountID='" & Acid & "',AccountName='" & AcName & "',Amount=" & Amt & ",DC='" & Dc & "',Remark='" & Remarks & "' where VourchersID=" & vchid & ""
            '    Ledger = ExecNonQuery(sSql)
        End If
    ...
    Public Shared Function FastCrateLedger(FastQuery) As Integer
        Dim sSql As String = String.Empty
        sSql = "Insert into CrateVoucher(VoucherID,SlipNo, EntryDate,TransType,AccountID,AccountName,CrateType,CrateID,CrateName, Qty,Remark,Rate,Amount,CashPaid,ServerTag,OrgID)" & FastQuery & ""
