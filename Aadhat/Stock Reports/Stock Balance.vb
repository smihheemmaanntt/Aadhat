Public Class Stock_Balance
    ...
    Private Sub mskFromDate_KeyDown(sender As Object, e As KeyEventArgs) Handles txtEntryDate.KeyDown
        If e.KeyCode = Keys.Enter Then
            SendKeys.Send("{TAB}")
        Else
            Exit Sub
    ...
    Private Sub Retrive(Optional ByVal condtion As String = "", Optional ByVal condtion1 As String = "")
        ...
        ssql = "Select StockHolderID,StockHolderName,StorageName,ItemID,AccountName, ItemName,sum(nug) as PurchaseNug From Purchase Where EntryDate <= '" & CDate(txtEntryDate.Text).ToString("yyyy-MM-dd") & "' " & condtion & "   Group by StockHolderID,ItemID order by StockHolderName,ItemName;"
        dt = clsFun.ExecDataTable(ssql)
        If dt.Rows.Count > 0 Then
            For i = 0 To dt.Rows.Count - 1
                Application.DoEvents()
                'If Val(dt.Rows(i)("StockHolderID")) = 1126 Then MsgBox("a")
                Dim SoldNug As String = clsFun.ExecScalarStr("Select sum(Nug) as SoldNug,lot from Transaction2 where Transtype in('Stock Sale','On Sale', 'Standard Sale','Store Out') and sallerID=" & Val(dt.Rows(i)("StockHolderID")) & " and ItemID=" & Val(dt.Rows(i)("ItemID")) & " and EntryDate <= '" & CDate(txtEntryDate.Text).ToString("yyyy-MM-dd") & "'")
    ...
    '                          "FROM Purchase WHERE EntryDate <= '" & CDate(txtEntryDate.Text).ToString("yyyy-MM-dd") & "' " & condtion & " " & _
    ...
    '                              "AND EntryDate <= '" & CDate(txtEntryDate.Text).ToString("yyyy-MM-dd") & "' " & _
    ...
    Private Sub printRecord()
        ...
        For Each row As DataGridViewRow In dg1.Rows
            With row
                sql = sql & "insert into Printing(D1,P1, P2,P3, P4, P5, P6,P7,P8,P9) values('" & txtEntryDate.Text & "','" & .Cells(1).Value & "'," & _
