Public Class Lot_Wise_Stock_Report
    ...
    Private Sub txtEntryDate_KeyDown(sender As Object, e As KeyEventArgs) Handles txtEntryDate.KeyDown, cbTrackStock.KeyDown, txtEntryDate.KeyDown
    ...
    Private Sub txtEntryDate_Validating(sender As Object, e As System.ComponentModel.CancelEventArgs) Handles txtEntryDate.Validating
        txtEntryDate.Text = SmartDate(txtEntryDate.Text, 2)
    End Sub
    ...
    Private Sub Retrive(Optional ByVal condtion As String = "", Optional ByVal condtion1 As String = "")
      ...
      " and PurchaseID=Purchase.VoucherID and EntryDate <= '" & CDate(txtEntryDate.Text).ToString("yyyy-MM-dd") & "')as soldNug,(Sum(Weight)-" & _
      " (Select ifnull(sum(Weight),0) from Transaction2 where Transtype in ('Stock Sale','On Sale','Standard Sale','Store Out')  and Lot=Purchase.LotNo and ItemID=Purchase.ItemID" & _
      " and PurchaseID=Purchase.VoucherID and EntryDate <= '" & CDate(txtEntryDate.Text).ToString("yyyy-MM-dd") & "')) as RestNug From Purchase" & _
      " Where EntryDate <= '" & CDate(txtEntryDate.Text).ToString("yyyy-MM-dd") & "' " & condtion & " " & condtion1 & " " & _
      " Group by ItemID,LotNo,VoucherID Having RestNug <> 0  order by EntryDate,AccountName,ItemName,LotNo "
    ...
    Private Sub printRecord()
        ...
        For Each row As DataGridViewRow In dg1.Rows
            With row
                sql = sql & "insert into Printing(D1,D2,P1, P2,P3, P4, P5, P6,P7,P8,P9,P10,P11) values('" & txtEntryDate.Text & "','" & .Cells(1).Value & "'," & _
