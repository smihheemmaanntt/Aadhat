...
Public Class Store_Transfer
    ...
    Private Sub Store_Transfer_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Else
            clsFun.FillDropDownList(CbStoreFrom, "Select  StorageID,StorageName From Purchase where StockHolderID=" & Val(cbAccountName.SelectedValue) & "  group by  StorageID,StorageName ", "StorageName", "StorageID", "")
        End If
        Me.BackColor = Color.FromArgb(247, 220, 111) : txtEntryDate.Text = Date.Today.ToString("dd-MM-yyyy")
        clsFun.FillDropDownList(CbLot, "Select VoucherID,LotNo,(ifnull(sum(Nug),0) -(Select ifnull(sum(Nug),0) From Transaction2 Where SallerID='" & Val(cbAccountName.SelectedValue) & "'  and ItemID = '" & Val(cbitemName.SelectedValue) & "' and TransType in ('Stock Sale','On Sale','Standard Sale','Store Out') and StorageID=" & Val(CbStoreFrom.SelectedValue) & " and EntryDate<='" & CDate(txtEntryDate.Text).ToString("yyyy-MM-dd") & "' )) as RestNug" &
                                  " from Purchase where EntryDate<='" & CDate(txtEntryDate.Text).ToString("yyyy-MM-dd") & "' and StorageID=" & Val(CbStoreFrom.SelectedValue) & " and StockHolderID=" & Val(cbAccountName.SelectedValue) & " and ItemID=" & Val(cbitemName.SelectedValue) & " Group By VoucherID having RestNug>0  order by LotNo ", "LotNo", "VoucherID", "")
    ...
    Private Sub cbitemName_GotFocus(sender As Object, e As EventArgs) Handles cbitemName.GotFocus
        ...
        clsFun.FillDropDownList(cbitemName, "Select ItemID,ItemName,(ifnull(sum(Nug),0) -(Select ifnull(sum(Nug),0) From Transaction2 Where SallerID=Purchase.StockHolderID  and ItemID = Purchase.ItemID and TransType in ('Stock Sale','On Sale','Standard Sale','Store Out') and StorageID=" & Val(CbStoreFrom.SelectedValue) & " and EntryDate<='" & CDate(txtEntryDate.Text).ToString("yyyy-MM-dd") & "' )) as RestNug" &
                                           " from Purchase where EntryDate<='" & CDate(txtEntryDate.Text).ToString("yyyy-MM-dd") & "' and StorageID=" & Val(CbStoreFrom.SelectedValue) & " and StockHolderID=" & Val(cbAccountName.SelectedValue) & "  Group By ItemID having RestNug>0  order by ItemName ", "ItemName", "ItemID", "")
    ...
    Private Sub ItemBalance()
        ...
        PurchaseBal = clsFun.ExecScalarStr("Select sum(Nug) From Purchase Where StockHolderID=" & Val(cbAccountName.SelectedValue) & " and StorageID=" & Val(CbStoreFrom.SelectedValue) & " and ItemID = " & Val(cbitemName.SelectedValue) & " and  EntryDate<='" & CDate(txtEntryDate.Text).ToString("yyyy-MM-dd") & "'")
        StockBal = clsFun.ExecScalarStr("Select sum(Nug) From Transaction2 Where SallerID=" & Val(cbAccountName.SelectedValue) & " and StorageID=" & Val(CbStoreFrom.SelectedValue) & "  and ItemID = " & Val(cbitemName.SelectedValue) & " and TransType  in ('Stock Sale','On Sale','Standard Sale','Store Out') and  EntryDate<='" & CDate(txtEntryDate.Text).ToString("yyyy-MM-dd") & "'")
        ...
        Else '''''''''''''''''''''''''''''for Update Stock--------------------------------------
            If dg1.RowCount = 0 Then ' if no rows addred
                bal = (RestBal)
            Else 'if rows count
                UpdateTmp = clsFun.ExecScalarInt("Select sum(Nug) From Transaction2 Where SallerID=" & Val(cbAccountName.SelectedValue) & "  and ItemID = " & Val(cbitemName.SelectedValue) & " and StorageID=" & Val(CbStoreFrom.SelectedValue) & " AND VoucherID not in ('" & Val(txtid.Text) & "') and TransType  in ('Stock Sale','On Sale','Standard Sale','Store Out') and  EntryDate<='" & CDate(txtEntryDate.Text).ToString("yyyy-MM-dd") & "'")
                If dg1.SelectedRows.Count = 0 Then
                    For i As Integer = 0 To dg1.RowCount - 1
                        If dg1.Rows(i).Cells(0).Value = Val(cbitemName.SelectedValue) Then
                            tmpbal = Val(tmpbal) + Val(dg1.Rows(i).Cells(4).Value)
    ...
    Private Sub Save()
        ...
        Try
            ...
            cmd.Parameters.AddWithValue("@1", CDate(txtEntryDate.Text).ToString("yyy-MM-dd"))
        ...
        For i As Integer = 0 To dg1.Rows.Count - 1
            With dg1.Rows(i)
                Dim ssql As String = "insert into Purchase(VoucherID,EntryDate, TransType,PurchaseTypeName,AccountID, AccountName,StockHolderID,StockHolderName, StorageID, StorageName,ItemID,ItemName,StockID,LotNo,Nug) " &
                                             " VALUES(" & Val(txtid.Text) & ",'" & CDate(txtEntryDate.Text).ToString("yyy-MM-dd") & "','Store In','Store In','" & Val(cbAccountName.SelectedValue) & "', " &
                ...
                ssql = ssql & "insert into Transaction2(VoucherID,EntryDate, TransType ,SallerID, SallerName, StorageID, StorageName,ItemID,ItemName,PurchaseID,Lot,Nug) " &
                             " VALUES(" & Val(txtid.Text) & ",'" & CDate(txtEntryDate.Text).ToString("yyy-MM-dd") & "','Store Out','" & Val(cbAccountName.SelectedValue) & "','" & cbAccountName.Text & "', " &
    ...
    Private Sub UpdateRecord()
        ...
        Sql = "Update Vouchers Set EntryDate='" & CDate(txtEntryDate.Text).ToString("yyy-MM-dd") & "', AccountID='" & Val(cbAccountName.SelectedValue) & "', " &
