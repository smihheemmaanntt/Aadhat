Public Class Store_Transfer_Register
    ...
    Private Sub rowColums()
        ...
        dg1.Columns(2).Name = "Vch No" : dg1.Columns(2).Width = 173
        dg1.Columns(3).Name = "Party Name" : dg1.Columns(3).Width = 310
        dg1.Columns(4).Name = "From Store" : dg1.Columns(4).Width = 100
        dg1.Columns(5).Name = "TO Store" : dg1.Columns(5).Width = 100
        dg1.Columns(6).Name = "Item Name" : dg1.Columns(6).Width = 108
        dg1.Columns(7).Name = "Lot No" : dg1.Columns(7).Width = 120
        dg1.Columns(8).Name = "Nugs" : dg1.Columns(8).Width = 160
    End Sub
    Sub calc()
        Dim i As Integer
        TxtGrandTotal.Text = Format(0, "0.00")
        For i = 0 To dg1.Rows.Count - 1
            TxtGrandTotal.Text = Val(TxtGrandTotal.Text) + Val(dg1.Rows(i).Cells(7).Value)
        Next
    End Sub
    Private Sub retrive(Optional ByVal condtion As String = "")
        Dim dt As New DataTable : dg1.Rows.Clear()
        dt = clsFun.ExecDataTable("Select V.ID as ID,V.BillNo as BillNo,V.EntryDate as EntryDate,V.AccountName as AccountName,V.StorageName,T1,P.ItemName as ItemName,P.LotNo as LotNo, " & _
                                  "P.Nug as Nug FROM Vouchers AS V INNER JOIN Purchase AS P ON P.VoucherID = V.ID WHERE " & _
                                  "V.EntryDate Between '" & CDate(Me.txtFromDate.Text).ToString("yyyy-MM-dd") & "' And '" & CDate(txtToDate.Text).ToString("yyyy-MM-dd") & "' " & _
                                   "" & condtion & " and V.transtype='" & Me.Text & "'Order By EntryDate")
            ...
            If dt.Rows.Count > 0 Then
                For i = 0 To dt.Rows.Count - 1
                    ...
                    With dg1.Rows(i)
                        ...
                        .Cells(5).Value = dt.Rows(i)("T1").ToString()
                        .Cells(6).Value = dt.Rows(i)("ItemName").ToString()
                        .Cells(7).Value = dt.Rows(i)("LotNo").ToString()
                        .Cells(8).Value = Val(dt.Rows(i)("Nug").ToString())
    ...
    Private Sub dg1_KeyDown(sender As Object, e As KeyEventArgs) Handles dg1.KeyDown
        If e.KeyCode = Keys.Enter Then
            ...
            Store_Transfer.Show()
            Store_Transfer.FillControls(tmpID)
    ...
    Private Sub PrintRecord()
        ...
        For Each row As DataGridViewRow In dg1.Rows
            With row
                sql = "insert into Printing(D1,D2, P1, P2,P3, P4, P5, P6,P7,P8,T1,T2,T3,M1) values('" & txtFromDate.Text & "','" & txtToDate.Text & "'," & _
                    ...
                    "'" & Format(Val(.Cells("Remark").Value), "0.00") & "'," & Format(Val(TxtGrandTotal.Text), "0.00") & ",'" & lblName.Text & "')"
                Try
