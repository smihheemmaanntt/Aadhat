...
Public Class PayMentform
    ...
    Public Sub FillPayment()
        ...
        If dt.Rows.Count > 0 Then
            If dt.Rows(0)("PymtDate").ToString().Trim() = "Yes" Then txtEntryDate.TabStop = True Else txtEntryDate.TabStop = False : txtMode.Focus()
    ...
    Private Sub ServerLedger()
        ...
        If Val(txtTotalAmount.Text) > 0 Then ''Total Amout
            If txtModeID.Text > 0 Then ''Party Account
                ...
                FastQuery = FastQuery & IIf(FastQuery <> "", " UNION ALL SELECT ", " SELECT ") & Val(txtID.Text) & ",'" & CDate(txtEntryDate.Text).ToString("yyyy-MM-dd") & "','" & Me.Text & "'," & Val(txtModeID.Text) & ",'" & txtMode.Text & "'," & Val(txtTotalAmount.Text) & ",'C'," & Val(ServerTag) & "," & Val(OrgID) & ",'" & Remark2 & IIf(TxtRemark.Text = "", "", ", Remark :" & TxtRemark.Text) & "','" & txtAccount.Text & "','" & RemarkHindi & IIf(TxtRemark.Text = "", "", ", Remark :" & TxtRemark.Text) & "'"
            End If
        End If
    ...
    Private Sub InsertLedger()
        ...
        If Val(txtTotalAmount.Text) > 0 Then ''Total Amout
            If txtModeID.Text > 0 Then ''Party Account
                '  clsFun.Ledger(0, Val(TxtID.text), SqliteEntryDate, Me.Text, txtModeID.Text, txtMode.Text, Val(txtTotalAmount.Text), "D", Remark1 & IIf(TxtRemark.Text = "", "", ", Remark :" & TxtRemark.Text), txtAccount.Text, RemarkHindi & IIf(TxtRemark.Text = "", "", ", Remark :" & TxtRemark.Text))
                FastQuery = FastQuery & IIf(FastQuery <> "", " UNION ALL SELECT ", " SELECT ") & Val(txtID.Text) & ",'" & CDate(txtEntryDate.Text).ToString("yyyy-MM-dd") & "','" & Me.Text & "'," & Val(txtModeID.Text) & ",'" & txtMode.Text & "'," & Val(txtTotalAmount.Text) & ",'C','" & "(" & txtAccount.Text & ") :" & Remark2 & IIf(TxtRemark.Text = "", "", ", Remark :" & TxtRemark.Text) & "','" & txtAccount.Text & "','" & "(" & txtAccount.Text & ") :" & RemarkHindi & IIf(TxtRemark.Text = "", "", ", Remark :" & TxtRemark.Text) & "'," & Val(txtAccountID.Text) & ",'" & txtAccount.Text & "'"
            End If
        End If
        If FastQuery = String.Empty Then Exit Sub
    ...
    Private Sub txtReciveAmount_GotFocus(sender As Object, e As EventArgs) Handles txtReciveAmount.GotFocus
        ...
        DgAccountSearch.Visible = False : AcBal()
    End Sub
    Private Sub txtMode_GotFocus(sender As Object, e As EventArgs) Handles txtMode.GotFocus, txtAccount.GotFocus, txtEntryDate.GotFocus,
    ...
    Private Sub ShowWhatsappContacts(Optional ByVal condtion As String = "")
        ...
        Dim i As Integer
        Dim count As Integer = 0
        Dim SSql As String = "Select * From Vouchers Where TransType='Payment' and EntryDate='" & CDate(txtEntryDate.Text).ToString("yyyy-MM-dd") & "'"
        dt = clsFun.ExecDataTable(SSql)
        If dt.Rows.Count > 0 Then
            For i = 0 To dt.Rows.Count - 1
                DgWhatsapp.Rows.Add()
                ...
                If .Cells(0).Value = True Then
                    If btnRadioEnglish.Checked = True And .Cells(3).Value <> "" Then
                        If RadioPDFMsg.Checked = True Then
                            GlobalData.PdfName = WhatsAppOfficialApi.CleanDocumentName(.Cells(4).Value.ToString() & "(" & .Cells(2).Value.ToString() & ")-" & txtEntryDate.Text, "Payment.pdf")
    ...
    Private Function CleanOfficialPaymentParameterFields(ByVal parameterFields As String) As String
        ...
        For Each fieldName As String In If(parameterFields, "").Split(","c)
            Dim key As String = NormalizeOfficialPaymentParameterFieldKey(fieldName)
    ...
    Private Function NormalizeOfficialPaymentParameterFieldKey(ByVal fieldKey As String) As String
    ...
    Private Function ExportPaymentPdfForOfficial(ByVal row As DataGridViewRow) As String
        ...
        GlobalData.PdfName = WhatsAppOfficialApi.CleanDocumentName(GetCellText(row, 4) & "(" & GetCellText(row, 2) & ")-" & txtEntryDate.Text, "Payment.pdf")
            ...
            With row
                ...
                If .Cells(0).Value = True AndAlso GetCellText(row, 3) <> "" Then
                    ...
                    Dim sent As Boolean = WhatsAppOfficialApi.SendSmartMessage(
                        ...
                        fields,
                        apiResponse,
                        WhatsAppOfficialApi.CleanDocumentName(GlobalData.PdfName, "Payment.pdf"))
    ...
    Private Sub WhatsAppDesktop()
        ...
        '            GlobalData.PdfName = .Cells(4).Value & "-" & txtEntryDate.Text & ".pdf"
