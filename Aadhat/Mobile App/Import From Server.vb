...
Public Class Import_From_Server
    ...
    '   Dim loginResp As String = String.Empty
    Dim httpService As New HttpService()
    ...
    Private Sub Import_From_Server_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        txtPassword.Text = clsFun.ExecScalarStr("Select Password From Company")
        Me.Top = 0 : Me.Left = 0
    ...
    Private Sub save()
                ...
                If .Cells(11).Value <> "Yes" Then
                    ...
                    Try
                        ...
                        If cmd.ExecuteNonQuery() > 0 Then
                            ...
                            vIds.Add(Convert.ToInt32(row.Cells(0).Value))
        ...
        Next
        If My.Computer.Network.IsAvailable = False Then MsgBox("Please Check Your Internet Connection", MsgBoxStyle.Critical, "No Internet Access") : Exit Sub
        ' Dim voucherIds As New List(Of Integer) From {id}
        Dim loginResp As LoginResponse = httpService.Authenticate(txtOrgID.Text, txtPassword.Text)
        httpService.DeleteVouchers(loginResp.Token, vIds)
        MsgBox("Imported Data Form Server Completed Successfully....", vbInformation, "Imported")
        LoadVouchers() ' retrive()
    End Sub
    Private Sub btnShow_Click(sender As Object, e As EventArgs) Handles btnShow.Click
        If My.Computer.Network.IsAvailable = False Then MsgBox("Please Check Your Internet Connection", MsgBoxStyle.Critical, "No Internet Access") : Exit Sub
        ' MsgBox("Updating Soon", MsgBoxStyle.Critical, "Comming Soon") : Exit Sub
        LoadVouchers() : LoadCrateVouchers()
        'retrive() : retrive2()
    End Sub

    Private Sub LoadCrateVouchers()
        Try
            dg2.Rows.Clear()
            Dim loginResp As LoginResponse = httpService.Authenticate(txtOrgID.Text, txtPassword.Text)
            Dim vouchers As List(Of CrateVoucher) = httpService.GetCrateVouchers(loginResp.Token)

            If vouchers IsNot Nothing AndAlso vouchers.Count > 0 Then
                If vouchers.Count > 10 Then dg2.Columns(10).Width = 180

                For i = 0 To vouchers.Count - 1
                    dg2.Rows.Add()
                    With dg2.Rows(i)
                        .Cells(0).Value = vouchers(i).Id
                        .Cells(1).Value = vouchers(i).SlipNo
                        .Cells(2).Value = Format(vouchers(i).EntryDate, "dd-MM-yyyy")
                        .Cells(3).Value = vouchers(i).AccountID
                        .Cells(4).Value = vouchers(i).AccountName
                        .Cells(5).Value = vouchers(i).CrateType
                        .Cells(6).Value = vouchers(i).CrateID
                        .Cells(7).Value = vouchers(i).CrateName
                        .Cells(8).Value = vouchers(i).Qty
                        .Cells(9).Value = vouchers(i).Rate
                        .Cells(10).Value = vouchers(i).Amount
                        .Cells(11).Value = If(vouchers(i).CashPaid, "Yes", "No")
                        .Cells(12).Value = vouchers(i).Remark
                        .Cells(13).Value = If(vouchers(i).IsCanceled, "Yes", "No")

                        ' बैकग्राउंड कलर सेट करना
                        If vouchers(i).CrateType = "Crate Out" Then
                            For j As Integer = 1 To 13
                                .Cells(j).Style.BackColor = Color.IndianRed
                            Next
                        ElseIf vouchers(i).IsCanceled Then
                            For j As Integer = 1 To 13
                                .Cells(j).Style.BackColor = Color.Coral
                            Next
                        End If
                    End With
                Next
    ...
    Private Sub LoadVouchers()
        Try
            dg1.Rows.Clear()
            Dim loginResp As LoginResponse = httpService.Authenticate(txtOrgID.Text, txtPassword.Text)
            Dim vouchers As List(Of Voucher) = httpService.GetVouchers(loginResp.Token)

            If vouchers IsNot Nothing AndAlso vouchers.Count > 0 Then
                If vouchers.Count > 10 Then dg1.Columns(9).Width = 180

                For i = 0 To vouchers.Count - 1
                    dg1.Rows.Add()
                    With dg1.Rows(i)
                        .Cells(0).Value = vouchers(i).Id
                        .Cells(1).Value = Format(vouchers(i).EntryDate, "dd-MM-yyyy")
                        .Cells(2).Value = vouchers(i).TransType
                        .Cells(3).Value = vouchers(i).Mode
                        .Cells(4).Value = vouchers(i).ReceiptNo
                        .Cells(5).Value = vouchers(i).AccountId
                        .Cells(6).Value = vouchers(i).AccountName
                        .Cells(7).Value = vouchers(i).Amount
                        .Cells(8).Value = vouchers(i).Discount
                        .Cells(9).Value = vouchers(i).Total
                        .Cells(10).Value = vouchers(i).Remark
                        .Cells(11).Value = If(vouchers(i).IsCanceled, "Yes", "No")
                        ' बैकग्राउंड कलर सेट करना
                        If vouchers(i).TransType = "Payment" Then
    ...
    Private Sub saveCrate()
        ...
        If My.Computer.Network.IsAvailable = False Then MsgBox("Please Check Your Internet Connection", MsgBoxStyle.Critical, "No Internet Access") : Exit Sub
       Dim loginResp As LoginResponse = httpService.Authenticate(txtOrgID.Text, txtPassword.Text)
        httpService.DeleteCrateVouchers(loginResp.Token, vIds)
        LoadCrateVouchers()
    ...
    Private Sub ButtonControl()
        ...
        Next
    End Sub
    Private Sub Button3_Click(sender As Object, e As EventArgs) Handles btnSaveVouchers.Click
    ...
    Private Sub Button2_Click(sender As Object, e As EventArgs) Handles Button2.Click
        If My.Computer.Network.IsAvailable = False Then MsgBox("Please Check Your Internet Connection", MsgBoxStyle.Critical, "No Internet Access") : Exit Sub
        ButtonControl() : saveCrate() : ButtonControl() : MsgBox("Imported Crate Form Server Completed Successfully....", vbInformation, "Imported")
    End Sub
