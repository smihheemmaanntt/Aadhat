...
Imports System.Text
Public Class Loose_Sale_Register
    ...
    Private Sub Loose_Sale_Register_FormClosing(sender As Object, e As FormClosingEventArgs) Handles Me.FormClosing
    ...
    Private Sub Loose_Sale_Register_KeyDown(sender As Object, e As KeyEventArgs) Handles Me.KeyDown
        If e.KeyCode = Keys.Escape Then
            If isBackgroundWorkerRunning Then
                Me.Hide()
                Me.Top = 0 : Me.Left = 0
            Else
                If MessageBox.Show("Are You Sure want to Exit Loose Sale Register ??", "Exit Confirmation...", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
                    Me.Close()
                End If
            End If
        End If
    ...
    Private Sub Loose_Sale_Register_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Me.Top = 0 : Me.Left = 0
    ...
    Private Sub retrive(Optional ByVal condtion As String = "")
        ...
        dt = clsFun.ExecDataTable("Select * FROM Vouchers WHERE EntryDate Between '" & CDate(mskFromDate.Text).ToString("yyyy-MM-dd") & "' And '" & CDate(MsktoDate.Text).ToString("yyyy-MM-dd") & "' and transtype='" & Me.Text & "'  " & condtion & " order by EntryDate,CAST(BillNo AS INTEGER)")
        Try
    ...
    End Sub
    Private Sub retrive1(Optional ByVal Primary As String = "", Optional ByVal Secondary As String = "")
        dg1.Rows.Clear()
        Dim dt As New DataTable
        ...
        dt = clsFun.ExecDataTable("Select * FROM Vouchers v   INNER JOIN   Transaction4 t ON v.id = t.VoucherID  WHERE V.EntryDate Between '" & CDate(mskFromDate.Text).ToString("yyyy-MM-dd") & "' And '" & CDate(MsktoDate.Text).ToString("yyyy-MM-dd") & "' and V.transtype='" & Me.Text & "' " & Primary & "" & Secondary & "  order by V.EntryDate")
    ...
    '                            "'" & .Cells(49).Value & "','" & .Cells(50).Value & "','" & .Cells(51).Value & "','" & .Cells(52).Value & "','" & .Cells(53).Value & "'," & _
    '                            "'" & .Cells(54).Value & "','" & .Cells(55).Value & "','" & .Cells(56).Value & "','" & .Cells(57).Value & "','" & .Cells(58).Value & "'," & _
    '                            "'" & .Cells(59).Value & "','" & .Cells(60).Value & "','" & .Cells(61).Value & "'"
    '            End If
    '        End With
    '    Next
    '    Try
    '        If FastQuery = String.Empty Then Exit Sub
    '        sql = "insert into Printing(D1, D2,M1, P1,P2, P3, P4, P5, P6,P7,P8,P9,P10,P11,P12,P13,P14,P15,P16,P17,P18,P19,P20, " & _
    '                     " P21,P22,P23,P24,P25,P26,P27,P28,P29,P30,P31,P32,P33,P34,P35,P36,P37,P38,P39,P40,P41,P42,P43,P44,P45,P46,P47,P48)" & FastQuery & ""
    '        ClsFunPrimary.ExecNonQuery(sql)
    '    Catch ex As Exception
    '        MsgBox(ex.Message)
    '        ClsFunPrimary.CloseConnection()
                ...
                If .Cells(6).Value <> "" Then
                    ...
                    FastQuery = FastQuery & IIf(FastQuery <> "", " UNION ALL SELECT ", " SELECT ") & "'" & .Cells(1).Value & "','" & .Cells(2).Value & "','" & .Cells(3).Value & "','" & .Cells(4).Value & "'," & _
                             "'" & .Cells(5).Value & "','" & .Cells(6).Value & "','" & .Cells(7).Value & "','" & .Cells(8).Value & "', " & _
                                ...
                                "'" & .Cells(49).Value & "','" & .Cells(50).Value & "','" & .Cells(51).Value & "','" & .Cells(52).Value & "','" & .Cells(53).Value & "'," & _
                                "'" & .Cells(54).Value & "','" & .Cells(55).Value & "','" & .Cells(56).Value & "','" & .Cells(57).Value & "','" & .Cells(58).Value & "'," & _
                                "'" & .Cells(59).Value & "','" & .Cells(60).Value & "','" & .Cells(61).Value & "','" & mskFromDate.Text & "','" & MsktoDate.Text & "'"
                End If
            End With
        Next
        Try
            If FastQuery = String.Empty Then Exit Sub
            sql = "insert into Printing(D1, D2,M1, P1,P2, P3, P4, P5, P6,P7,P8,P9,P10,P11,P12,P13,P14,P15,P16,P17,P18,P19,P20, " & _
                         " P21,P22,P23,P24,P25,P26,P27,P28,P29,P30,P31,P32,P33,P34,P35,P36,P37,P38,P39,P40,P41,P42,P43,P44,P45,P46,P47,P48,M9,M10)" & FastQuery & ""
            ClsFunPrimary.ExecNonQuery(sql)
        Catch ex As Exception
            MsgBox(ex.Message)
            ClsFunPrimary.CloseConnection()
    ...
    Private Sub PrintRegister2()
        ...
        For i As Integer = 0 To maxRowCount - 1
            ...
            For LastCount = 0 To IIf(i = (maxRowCount - 1), Val(TotalRecord - 1), 99)
                With dg1.Rows(LastRecord)
                    FastQuery = FastQuery & IIf(FastQuery <> "", " UNION ALL SELECT ", " SELECT ") & "'" & mskFromDate.Text & "','" & MsktoDate.Text & "'," & _
                        "'" & .Cells("Date").Value & "','" & .Cells("No.").Value & "','" & .Cells("Seller").Value & "','" & .Cells("Item").Value & "','" & .Cells("Customer").Value & "'," & _
            ...
            Try
                If FastQuery = String.Empty Then Exit Sub
                sQL = "insert into Printing(D1,D2,M1,M2, P1, P2,P3, P4, P5, P6,P7,P8,P9,P10,T1,T2,T3,T4,T5) " & FastQuery & ""
                ClsFunPrimary.ExecNonQuery(sQL)
            Catch ex As Exception
                MsgBox(ex.Message)
                ClsFunPrimary.CloseConnection()
    ...
    Private Sub btnPrint_Click(sender As Object, e As EventArgs) Handles BtnPrint.Click
        If ckShowItems.Checked = True Then
            PrintRegister2()
            Report_Viewer.printReport("\Reports\LooseRegister2.rpt")
            Report_Viewer.MdiParent = MainScreenForm
            Report_Viewer.Show()
            Ugrahi_Viewer.BringToFront()
        Else
            PrintRegister()
            Report_Viewer.printReport("\Reports\LooseRegister.rpt")
            Report_Viewer.MdiParent = MainScreenForm
            Report_Viewer.Show()
            Ugrahi_Viewer.BringToFront()
        End If
    ...
    Sub retrive2(ByVal id As String, Optional ByVal condtion As String = "")
        ...
        dt = clsFun.ExecDataTable("Select Vouchers.ID, Vouchers.Entrydate, Vouchers.billNo,Vouchers.AccountID, Vouchers.AccountName,Accounts.Address,Accounts.City,Accounts.State,Accounts.Mobile1,Accounts.Mobile2, Vouchers.VehicleNo," _
                                  ...
                                  & "INNER JOIN Items ON Transaction4.ItemID = Items.ID) INNER JOIN Accounts ON Vouchers.AccountID = Accounts.ID  Where  Vouchers.EntryDate Between '" & CDate(mskFromDate.Text).ToString("yyyy-MM-dd") & "' And '" & CDate(MsktoDate.Text).ToString("yyyy-MM-dd") & "' " & id & " " & condtion & "")
        If dt.Rows.Count = 0 Then Exit Sub
        If dt.Rows.Count > 0 Then
            For i = 0 To dt.Rows.Count - 1
                ''''''''''''''''''''' Opening Balance'''''''''''''''''''''''''''''''''''
                opbal = Val(clsFun.ExecScalarStr("Select Round((Case When DC='Dr' then (ifnull(opbal,0)+(Select ifnull(Round(Sum(Amount),2),0) From Ledger Where AccountID=Accounts.ID and DC='D' and Ledger.Entrydate <'" & CDate(MsktoDate.Text).ToString("yyyy-MM-dd") & "')" & _
                ...
                ClBal = Val(clsFun.ExecScalarStr("Select Round((Case When DC='Dr' then (ifnull(opbal,0)+(Select ifnull(Round(Sum(Amount),2),0) From Ledger Where AccountID=Accounts.ID and DC='D' and Ledger.Entrydate <='" & CDate(MsktoDate.Text).ToString("yyyy-MM-dd") & "')" & _
                                         "-(Select ifnull(Round(Sum(Amount),2),0) From Ledger Where AccountID=Accounts.ID and DC='C' and Ledger.Entrydate <='" & CDate(MsktoDate.Text).ToString("yyyy-MM-dd") & "')) " & _
                                         " else (ifnull(-(opbal),0)+-(Select ifnull(Round(Sum(Amount),2),0) From Ledger Where AccountID=Accounts.ID and DC='C' and Ledger.Entrydate <='" & CDate(MsktoDate.Text).ToString("yyyy-MM-dd") & "')" & _
                                         " +(Select ifnull(Round(Sum(Amount),2),0) From Ledger Where AccountID=Accounts.ID and DC='D' and Ledger.Entrydate <='" & CDate(MsktoDate.Text).ToString("yyyy-MM-dd") & "'))  end),2) as  Restbal from Accounts Where RestBal<>0 and ID=" & Val(dt.Rows(i)("AccountID").ToString()) & " Order by upper(AccountName) ;"))
                ...
                Try
                    If dtcrate.Rows.Count > 0 Then
                        For U = 0 To dtcrate.Rows.Count - 1
                            '  If Application.OpenForms().OfType(Of Loose_Sale_Register).Any = False Then Exit Sub
                ...
                With tmpgrid.Rows(cnt)
                    ...
                    .Cells(39).Value = Val(clsFun.ExecScalarStr("Select Sum(Nug) From Transaction4 Where VoucherID= " & Val(dt.Rows(i)("ID").ToString()) & ""))
                    .Cells(40).Value = Format(Val(clsFun.ExecScalarStr("Select Sum(Weight) From Transaction4 Where VoucherID= " & Val(dt.Rows(i)("ID").ToString()) & "")), "0.00") 'Format(Val(dt.Compute("Sum(Weight)", "")), "0.00")
                    .Cells(41).Value = Format(Val(clsFun.ExecScalarStr("Select Sum(CommAmt) From Transaction4 Where VoucherID= " & Val(dt.Rows(i)("ID").ToString()) & "")), "0.00") 'Format(Val(dt.Compute("Sum(CommAmt)", "")), "0.00")
                    .Cells(42).Value = Format(Val(clsFun.ExecScalarStr("Select Sum(Mamt) From Transaction4 Where VoucherID= " & Val(dt.Rows(i)("ID").ToString()) & "")), "0.00") 'Format(Val(dt.Compute("Sum(Mamt)", "")), "0.00")
                    .Cells(43).Value = Format(Val(clsFun.ExecScalarStr("Select Sum(rdfAmt) From Transaction4 Where VoucherID= " & Val(dt.Rows(i)("ID").ToString()) & "")), "0.00") 'Format(Val(dt.Compute("Sum(rdfAmt)", "")), "0.00")
                    .Cells(44).Value = Format(Val(clsFun.ExecScalarStr("Select Sum(tareamt) From Transaction4 Where VoucherID= " & Val(dt.Rows(i)("ID").ToString()) & "")), "0.00") 'Format(Val(dt.Compute("Sum(tareamt)", "")), "0.00")
                    .Cells(45).Value = Format(Val(clsFun.ExecScalarStr("Select Sum(Labouramt) From Transaction4 Where VoucherID= " & Val(dt.Rows(i)("ID").ToString()) & "")), "0.00") 'Format(Val(dt.Compute("Sum(Labouramt)", "")), "0.00")
    ...
    '        id = " and Vouchers.id in (" & id & ")"
    '    End If
    '    dt = clsFun.ExecDataTable("Select Vouchers.ID, Vouchers.Entrydate, Vouchers.billNo,Vouchers.AccountID, Vouchers.AccountName, Vouchers.VehicleNo," _
    '                              & "Transaction4.ItemName, Transaction4.Lot, Transaction4.Nug as TransNug, Transaction4.Weight, Transaction4.Rate," _
    '                              & "Transaction4.Per, Transaction4.Amount,Transaction4.CommPer,Transaction4.CommAmt,Transaction4.MPer,Transaction4.MAmt," _
    '                              & "Transaction4.RdfPer,Transaction4.RdfAmt,Transaction4.Tare,Transaction4.TareAmt,Transaction4.labour,Transaction4.LabourAmt," _
    ...
    '                .Cells(37).Value = .Cells(37).Value & Format(Val(dt.Rows(i)("SubTotal").ToString()), "0.00")
    '                .Cells(38).Value = .Cells(38).Value & Format(Val(dt.Rows(i)("RoundOff").ToString()), "0.00")
    '                .Cells(39).Value = Format(Val(clsFun.ExecScalarStr("Select Sum(Nug) From Transaction4 Where VoucherID= " & Val(dt.Rows(i)("ID").ToString()) & "")), "0.00")
    '                .Cells(40).Value = Format(Val(clsFun.ExecScalarStr("Select Sum(Weight) From Transaction4 Where VoucherID= " & Val(dt.Rows(i)("ID").ToString()) & "")), "0.00") 'Format(Val(dt.Compute("Sum(Weight)", "")), "0.00")
    '                .Cells(41).Value = Format(Val(clsFun.ExecScalarStr("Select Sum(CommAmt) From Transaction4 Where VoucherID= " & Val(dt.Rows(i)("ID").ToString()) & "")), "0.00") 'Format(Val(dt.Compute("Sum(CommAmt)", "")), "0.00")
    '                .Cells(42).Value = Format(Val(clsFun.ExecScalarStr("Select Sum(Mamt) From Transaction4 Where VoucherID= " & Val(dt.Rows(i)("ID").ToString()) & "")), "0.00") 'Format(Val(dt.Compute("Sum(Mamt)", "")), "0.00")
    '                .Cells(43).Value = Format(Val(clsFun.ExecScalarStr("Select Sum(rdfAmt) From Transaction4 Where VoucherID= " & Val(dt.Rows(i)("ID").ToString()) & "")), "0.00") 'Format(Val(dt.Compute("Sum(rdfAmt)", "")), "0.00")
    ...
    Private Sub btnPrintBills_Click(sender As Object, e As EventArgs) Handles btnPrintBills.Click
        ...
        Else
            PrintRecord()
            If Application.OpenForms().OfType(Of Loose_Sale_Register).Any = False Then Exit Sub
            Report_Viewer.printReport("\LooseSale All.rpt")
            Report_Viewer.MdiParent = MainScreenForm
            Report_Viewer.Show()
    ...
    Private Sub btnPrint2_Click(sender As Object, e As EventArgs) Handles btnPrint2.Click
        ...
        Else
            PrintRecord()
            If Application.OpenForms().OfType(Of Loose_Sale_Register).Any = False Then Exit Sub
            Report_Viewer.printReport("\LooseSale2 All.rpt")
            Report_Viewer.MdiParent = MainScreenForm
            Report_Viewer.Show()
            If Not Report_Viewer Is Nothing Then
                Report_Viewer.BringToFront()
    ...
    Private Sub SendWhatsappData()
            ...
            With row
                UpdateProgressBar(row.Index)
                    If btnRadioEnglish.Checked = True AndAlso RadioPDFMsg.Checked = True Then
                        ...
                        GlobalData.PdfName = .Cells(2).Value.Replace("/", "") & "-" & mskFromDate.Text & ".pdf"
                        PrintRecord()
                        Pdf_Genrate.ExportReport("\Formats\LooseSale.rpt")
                ...
                ElseIf RadioRegional.Checked = True AndAlso RadioPDFMsg.Checked = True Then
                    ...
                    GlobalData.PdfName = .Cells(2).Value.Replace("/", "") & "-" & mskFromDate.Text & ".pdf"
                    PrintRecord()
                    Pdf_Genrate.ExportReport("\Formats\LooseSale2.rpt")
                    whatsappSender.FilePath = whatsappSender.UploadFile(Application.StartupPath & "\Pdfs\" & GlobalData.PdfName)
        ...
        Try
            ...
            If selectedIndex = 0 Then ClsFunWhatsapp.ExecNonQuery(Sql) Else WABA.ExecNonQuery(Sql)
    ...
    Private Sub ShowWhatsappContacts(Optional ByVal condtion As String = "")
        ...
        Dim i As Integer
        Dim count As Integer = 0
        ssql = "Select VoucherID,AccountID, (Select AccountName From Accounts Where ID=Transaction4.AccountID) as AccountName, ' Nug : '||sum(nug)||', Weight : '|| sum(weight) ||', Basic : '|| " &
                ...
                "(Select OtherName From Accounts Where ID=Transaction4.AccountID) as OtherName from Transaction4 " &
                ...
                " and TransType='" & Me.Text & "'" & condtion & " Group by AccountID order by accountName "
    ...
    Sub RowColumsWhatsapp()
        ...
        DgWhatsapp.Columns(7).Name = "Status" : DgWhatsapp.Columns(7).Width = 200
        ...
        Try
            If dt.Rows.Count > 0 Then
                For i = 0 To dt.Rows.Count - 1
                    ...
                    If SendingMethod = "Easy WhatsApp" Then cbType.SelectedIndex = 1 Else cbType.SelectedIndex = 0 : cbType.Visible = True
    ...
    Private Sub Button5_Click(sender As Object, e As EventArgs) Handles Button5.Click
        FillControl()
        If ClsFunPrimary.ExecScalarStr("Select InstanceID From API") = "Easy Whatsapp" Then
        ...
        Else
            cbType.Visible = True : cbType.SelectedIndex = 0
            If DgWhatsapp.ColumnCount = 0 Then RowColumsWhatsapp() : ShowWhatsappContacts()
            pnlWhatsapp.Visible = True : cbType.SelectedIndex = 1
        End If
