...
Public Class Loose_Sale
    Dim vno As Integer : Dim VchId As Integer
    Dim sql As String = String.Empty : Dim count As Integer = 0
    Dim MaxID As String = String.Empty : Dim CalcType As String = String.Empty
    Dim LotBal As String = String.Empty : Dim TotalPages As Integer = 0 : Dim PageNumber As Integer = 0
    Dim RowCount As Integer = 1 : Dim Offset As Integer = 0
    Dim remark2 As String : Dim remarkHindi As String
    Dim ServerTag As Integer : Dim APIResposne As String
    ...
    Private Sub btnClose_Click(sender As Object, e As EventArgs) Handles btnClose.Click
        If isBackgroundWorkerRunning Then
            ...
            Me.Top = 0 : Me.Left = 0
        Else
            If MessageBox.Show("Are You Sure want to Exit Loose Sale ??", "Exit Confirmation...", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
                Me.Close()
            End If
        End If
    End Sub

    Private Sub Loose_Sale_FormClosing(sender As Object, e As FormClosingEventArgs) Handles Me.FormClosing
        If isBackgroundWorkerRunning Then
            e.Cancel = True
            Me.Hide()
            Me.Top = 0 : Me.Left = 0
    ...
    Public Sub FillstdSale()
        ...
        If dt.Rows.Count > 0 Then
            ...
            End If
            Cbper.Text = "Kg"
            ' If dt.Rows(0)("StdTaxPaid").ToString().Trim() = "Yes" Then ckTaxPaid.Visible = True Else ckTaxPaid.Visible = False
        End If
    End Sub
    ...
    Sub retrive2()
        ...
        '                          & "Transaction4.ItemName,  Transaction4.Nug as TransNug, Transaction4.Weight, Transaction4.Rate," _
        '                          & "Transaction4.Per, Transaction4.Amount,Transaction4.CommPer,Transaction4.CommAmt,Transaction4.MPer,Transaction4.MAmt," _
        '                          & "Transaction4.RdfPer,Transaction4.RdfAmt,Transaction4.Tare,Transaction4.TareAmt,Transaction4.labour,Transaction4.LabourAmt," _
        ...
        '                          & "Items.OtherName, Accounts.OtherName as AccountOtherName,Transaction4.Cratemarka as CrateMarka, Transaction4.CrateQty as CrateQty FROM ((Vouchers INNER JOIN Transaction4 ON Vouchers.ID = Transaction4.VoucherID)" _
        '                          & "INNER JOIN Items ON Transaction4.ItemID = Items.ID) INNER JOIN Accounts ON Vouchers.AccountID = Accounts.ID  Where (Vouchers.ID=" & Val(txtid.Text) & ")")
        ...
        dt = clsFun.ExecDataTable("Select Vouchers.ID, Vouchers.Entrydate, Vouchers.billNo,Vouchers.AccountID, Vouchers.AccountName,Accounts.Address,Accounts.City,Accounts.State,Accounts.Mobile1,Accounts.Mobile2, Vouchers.VehicleNo," _
                          ...
                          & "Transaction4.Per, Transaction4.Amount,Transaction4.CommPer,Transaction4.CommAmt,Transaction4.MPer,Transaction4.MAmt," _
                          & "Transaction4.RdfPer,Transaction4.RdfAmt,Transaction4.Tare,Transaction4.TareAmt,Transaction4.labour,Transaction4.LabourAmt," _
                          & " Transaction4.TotalAmount as TotAmt,Vouchers.Nug, Vouchers.Kg, Vouchers.BasicAmount, Vouchers.TotalAmount, Vouchers.DiscountAmount, Vouchers.TotalCharges, vouchers.SubTotal, " _
                          & "Vouchers.RoundOff,Vouchers.T1,Vouchers.T2,Vouchers.T3,Vouchers.T4,Vouchers.T5,Vouchers.T6,Vouchers.T7,Vouchers.T8,Vouchers.T9,Vouchers.T10, " _
                          & "Items.OtherName, Accounts.OtherName as AccountOtherName,Transaction4.Cratemarka as CrateMarka, Transaction4.CrateQty as CrateQty FROM ((Vouchers INNER JOIN Transaction4 ON Vouchers.ID = Transaction4.VoucherID)" _
                          & "INNER JOIN Items ON Transaction4.ItemID = Items.ID) INNER JOIN Accounts ON Vouchers.AccountID = Accounts.ID  Where (Vouchers.ID=" & Val(txtid.Text) & ")")

        ''''''''''''''''''''' Opening Balance'''''''''''''''''''''''''''''''''''
        opbal = Val(clsFun.ExecScalarStr("Select Round((Case When DC='Dr' then (ifnull(opbal,0)+(Select ifnull(Round(Sum(Amount),2),0) From Ledger Where AccountID=Accounts.ID and DC='D' and Ledger.Entrydate <'" & CDate(txtEntryDate.Text).ToString("yyyy-MM-dd") & "')" & _
                                 ...
                                 " +(Select ifnull(Round(Sum(Amount),2),0) From Ledger Where AccountID=Accounts.ID and DC='D' and Ledger.Entrydate <'" & CDate(txtEntryDate.Text).ToString("yyyy-MM-dd") & "'))  end),2) as  Restbal from Accounts Where RestBal<>0 and ID=" & Val(txtAccountID.Text) & " Order by upper(AccountName) ;"))

        ''''''''''''''''''''closing balance'''''''''''''''''''''''''

        ClBal = Val(clsFun.ExecScalarStr("Select Round((Case When DC='Dr' then (ifnull(opbal,0)+(Select ifnull(Round(Sum(Amount),2),0) From Ledger Where AccountID=Accounts.ID and DC='D' and Ledger.Entrydate <='" & CDate(txtEntryDate.Text).ToString("yyyy-MM-dd") & "')" & _
                                 "-(Select ifnull(Round(Sum(Amount),2),0) From Ledger Where AccountID=Accounts.ID and DC='C' and Ledger.Entrydate <='" & CDate(txtEntryDate.Text).ToString("yyyy-MM-dd") & "')) " & _
                                 " else (ifnull(-(opbal),0)+-(Select ifnull(Round(Sum(Amount),2),0) From Ledger Where AccountID=Accounts.ID and DC='C' and Ledger.Entrydate <='" & CDate(txtEntryDate.Text).ToString("yyyy-MM-dd") & "')" & _
                                 " +(Select ifnull(Round(Sum(Amount),2),0) From Ledger Where AccountID=Accounts.ID and DC='D' and Ledger.Entrydate <='" & CDate(txtEntryDate.Text).ToString("yyyy-MM-dd") & "'))  end),2) as  Restbal from Accounts Where RestBal<>0 and ID=" & Val(txtAccountID.Text) & " Order by upper(AccountName) ;"))

        TodaysCredit = Val(clsFun.ExecScalarStr("Select sum(Amount) as tot from Ledger where Dc='C' and accountID=" & Val(txtAccountID.Text) & " and EntryDate = '" & CDate(txtEntryDate.Text).ToString("yyyy-MM-dd") & "'"))
        todaysDebit = Val(clsFun.ExecScalarStr("Select sum(Amount) as tot from Ledger where Dc='D' and accountID=" & Val(txtAccountID.Text) & " and EntryDate = '" & CDate(txtEntryDate.Text).ToString("yyyy-MM-dd") & "'"))
        If ClBal < 0 Then
        ...
        " (Select ifnull(Sum(Qty),0) from CrateVoucher Where AccountID =  ACG.ID and CV.CrateID = CrateID and CrateType='Crate In' and EntryDate <= '" & CDate(txtEntryDate.Text).ToString("yyyy-MM-dd") & "')) as Reciveable," & _
                    " ((Select ifnull(Sum(Qty),0) from CrateVoucher Where AccountID = ACG.ID and CV.CrateID = CrateID and CrateType='Crate Out' and EntryDate <= '" & CDate(txtEntryDate.Text).ToString("yyyy-MM-dd") & "') -" & _
        " (Select ifnull(Sum(Qty),0) from CrateVoucher Where AccountID =  ACG.ID and CV.CrateID = CrateID and CrateType='Crate In' and EntryDate <= '" & CDate(txtEntryDate.Text).ToString("yyyy-MM-dd") & "')) as DueCrates " & _
        " FROM CrateVoucher as CV INNER JOIN Account_AcGrp AS ACG ON CV.AccountID = ACG.ID Where EntryDate <= '" & CDate(txtEntryDate.Text).ToString("yyyy-MM-dd") & "' and AccountID='" & Val(txtAccountID.Text) & "' Group by AccountID,CrateID Having DueCrates<>0 order by upper(ACG.AccountName);")
        Try
            If dtcrate.Rows.Count > 0 Then
                For U = 0 To dtcrate.Rows.Count - 1
                    '    If Application.OpenForms().OfType(Of Loose_Sale).Any = False Then Exit Sub
        ...
        End Try
        'dt = clsFun.ExecDataTable("Select Vouchers.Entrydate, Vouchers.billNo, Vouchers.AccountName, Vouchers.VehicleNo, Transaction4.ItemName, Transaction4.Lot, " _
        '                         & " Transaction4.Nug, Transaction4.Weight, Transaction4.Rate, Transaction4.Per, Transaction4.Amount, Vouchers.Nug, Vouchers.Kg, " _
        '                         & " Vouchers.BasicAmount, Vouchers.TotalAmount, Vouchers.TotalCharges, Accounts.OtherName as AccountOtherName, Items.OtherName, " _
        '                         & "Transaction4.CommPer, Transaction4.CommAmt, Transaction4.MPer, Transaction4.MAmt, Transaction4.RdfPer, Transaction4.RdfAmt, " _
        '                         & "Transaction4.Tare, Transaction4.Tareamt, Transaction4.labour, Transaction4.LabourAmt,Vouchers.ID,Vouchers.DiscountAmount, Vouchers.SubTotal FROM Items INNER JOIN " _
        '                         & "(Accounts INNER JOIN (Vouchers INNER JOIN Transaction4 ON Vouchers.ID = Transaction4.VoucherID) ON " _
        '                         & " Accounts.ID = Vouchers.AccountID) ON Items.ID = Transaction4.ItemID  Where (Vouchers.ID=" & Val(txtid.Text) & ")")
        '  tmpgrid.Rows.Clear()
    ...
    Private Sub Dg1Record()
        ...
        Try
            ...
            Sql = "insert into Transaction4(EntryDate,VoucherID,TransType,AccountID,AccountName,ItemID,ItemName,Nug,Weight, " _
            & " Rate, Per,Amount,Charges,TotalAmount,CommPer,CommAmt,MPer,MAmt,RdfPer,RdfAmt," _
            & "Tare,TareAmt,Labour, LabourAmt,CrateID,Cratemarka,CrateQty, MaintainCrate,CrateAccountID,CrateAccountName,PurchaseID,RoundOff) " & FastQuery & ""
    ...
    Private Sub insertLedger()
        ...
        For Each row As DataGridViewRow In dg1.Rows
            ...
            With row
                If .Cells(0).Value <> "" Then
                    remark2 = remark2 & txtVehicleNo.Text & " " & .Cells(0).Value & " Nug : " & .Cells(2).Value & " Weight : " & .Cells(3).Value & " On : " & .Cells(4).Value & " Per  /- " & .Cells(5).Value & ", Charges : " & .Cells(23).Value & ", Amount : " & .Cells(6).Value & vbCrLf
                    remarkHindi = remarkHindi & txtVehicleNo.Text & " " & clsFun.ExecScalarStr("Select OtherName From Items Where ID=" & (.Cells(8).Value) & "") & ", नग : " & .Cells(2).Value & " वजन : " & .Cells(3).Value & " भाव : " & .Cells(4).Value & "  /- " & .Cells(5).Value & ", ख़र्चे : " & .Cells(23).Value & vbCrLf
        ...
        If Val(txtAccountID.Text) > 0 Then ''Party Account
            '  clsFun.Ledger(0, Val(txtid.Text), CDate(txtEntryDate.Text).ToString("yyyy-MM-dd"), Me.Text, Val(txtAccountID.Text), txtAccount.Text, Val(txtTotalNetAmount.Text), "D", remark2, txtAccount.Text, remarkHindi)
            FastQuery = FastQuery & IIf(FastQuery <> "", " UNION ALL SELECT ", " SELECT ") & Val(txtid.Text) & ",'" & CDate(txtEntryDate.Text).ToString("yyyy-MM-dd") & "','" & Me.Text & "','" & Val(txtAccountID.Text) & "','" & txtAccount.Text & "','" & Val(txtTotalNetAmount.Text) & "','D','" & remark2 & "','','" & remarkHindi & "'"
        End If
        ''Maal Khata Account
        If Val(txtbasicTotal.Text) > 0 Then ''Maal Khata Account
            ...
            FastQuery = FastQuery & IIf(FastQuery <> "", " UNION ALL SELECT ", " SELECT ") & Val(txtid.Text) & ",'" & CDate(txtEntryDate.Text).ToString("yyyy-MM-dd") & "','" & Me.Text & "','" & Val(29) & "','" & clsFun.ExecScalarStr("Select AccountName from Accounts where Id=29") & "','" & Val(txtbasicTotal.Text) & "','C','" & remark2 & "','','" & remarkHindi & "'"
        End If

        If Val(CommAmt) > 0 Then ''Commission Account
            FastQuery = FastQuery & IIf(FastQuery <> "", " UNION ALL SELECT ", " SELECT ") & Val(txtid.Text) & ",'" & CDate(txtEntryDate.Text).ToString("yyyy-MM-dd") & "','" & Me.Text & "','" & Val(10) & "','" & clsFun.ExecScalarStr("Select AccountName from Accounts where Id=10") & "','" & Val(CommAmt) & "','C','" & remark2 & "','','" & remarkHindi & "'"
        End If
        If Val(MandiTax) > 0 Then ''Mandi Account
            FastQuery = FastQuery & IIf(FastQuery <> "", " UNION ALL SELECT ", " SELECT ") & Val(txtid.Text) & ",'" & CDate(txtEntryDate.Text).ToString("yyyy-MM-dd") & "','" & Me.Text & "','" & Val(30) & "','" & clsFun.ExecScalarStr("Select AccountName from Accounts where Id=30") & "','" & Val(MandiTax) & "','C','" & remark2 & "','','" & remarkHindi & "'"
        End If
        If Val(RDFAmt) > 0 Then ''rdf Account
            FastQuery = FastQuery & IIf(FastQuery <> "", " UNION ALL SELECT ", " SELECT ") & Val(txtid.Text) & ",'" & CDate(txtEntryDate.Text).ToString("yyyy-MM-dd") & "','" & Me.Text & "','" & Val(39) & "','" & clsFun.ExecScalarStr("Select AccountName from Accounts where Id=39") & "','" & Val(RDFAmt) & "','C','" & remark2 & "','','" & remarkHindi & "'"
        End If
        If Val(TareAmt) > 0 Then ''Bardana Account
            FastQuery = FastQuery & IIf(FastQuery <> "", " UNION ALL SELECT ", " SELECT ") & Val(txtid.Text) & ",'" & CDate(txtEntryDate.Text).ToString("yyyy-MM-dd") & "','" & Me.Text & "','" & Val(4) & "','" & clsFun.ExecScalarStr("Select AccountName from Accounts where Id=4") & "','" & Val(TareAmt) & "','C','" & remark2 & "','','" & remarkHindi & "'"
        End If
        If Val(Labouramt) > 0 Then ''Labour Account
            FastQuery = FastQuery & IIf(FastQuery <> "", " UNION ALL SELECT ", " SELECT ") & Val(txtid.Text) & ",'" & CDate(txtEntryDate.Text).ToString("yyyy-MM-dd") & "','" & Me.Text & "','" & Val(23) & "','" & clsFun.ExecScalarStr("Select AccountName from Accounts where Id=23") & "','" & Val(Labouramt) & "','C','" & remark2 & "','','" & remarkHindi & "'"
        End If
        If Val(Roff) <> 0 Then ''Account 
            If Val(Roff) < 0 Then
                FastQuery = FastQuery & IIf(FastQuery <> "", " UNION ALL SELECT ", " SELECT ") & Val(txtid.Text) & ",'" & CDate(txtEntryDate.Text).ToString("yyyy-MM-dd") & "','" & Me.Text & "'," & 42 & ",'" & clsFun.ExecScalarStr("Select AccountName from Accounts where Id=42") & "'," & Math.Abs(Val(Roff)) & ",'D' ,'" & Remark & "','" & txtAccount.Text & "','" & remarkHindi & "'"
            Else
                FastQuery = FastQuery & IIf(FastQuery <> "", " UNION ALL SELECT ", " SELECT ") & Val(txtid.Text) & ",'" & CDate(txtEntryDate.Text).ToString("yyyy-MM-dd") & "','" & Me.Text & "'," & 42 & ",'" & clsFun.ExecScalarStr("Select AccountName from Accounts where Id=42") & "'," & Val(Roff) & ",'C' ,'" & Remark & "','" & txtAccount.Text & "','" & remarkHindi & "'"
            End If
        End If
        If FastQuery = String.Empty Then Exit Sub
    ...
    Private Sub ServerinsertLedger()
        ...
        If Val(txtAccountID.Text) > 0 Then ''Party Account
            '  clsFun.Ledger(0, Val(txtid.Text), CDate(txtEntryDate.Text).ToString("yyyy-MM-dd"), Me.Text, Val(txtAccountID.Text), txtAccount.Text, Val(txtTotalNetAmount.Text), "D", remark2, txtAccount.Text, remarkHindi)
            FastQuery = FastQuery & IIf(FastQuery <> "", " UNION ALL SELECT ", " SELECT ") & Val(txtid.Text) & ",'" & CDate(txtEntryDate.Text).ToString("yyyy-MM-dd") & "','" & Me.Text & "','" & Val(txtAccountID.Text) & "','" & txtAccount.Text & "','" & Val(txtTotalNetAmount.Text) & "','D'," & Val(ServerTag) & "," & Val(OrgID) & ",'" & remark2 & "','','" & remarkHindi & "'"
        End If
        ''Maal Khata Account
        If Val(txtbasicTotal.Text) > 0 Then ''Maal Khata Account
            '  clsFun.Ledger(0, Val(txtid.Text), SqliteEntryDate, Me.Text, 29, clsFun.ExecScalarStr("Select AccountName from Accounts where Id=29"), Val(txtbasicTotal.Text), "C", remark2, txtAccount.Text, remarkHindi)
            FastQuery = FastQuery & IIf(FastQuery <> "", " UNION ALL SELECT ", " SELECT ") & Val(txtid.Text) & ",'" & CDate(txtEntryDate.Text).ToString("yyyy-MM-dd") & "','" & Me.Text & "','" & Val(29) & "','" & clsFun.ExecScalarStr("Select AccountName from Accounts where Id=29") & "','" & Val(txtbasicTotal.Text) & "','C'," & Val(ServerTag) & "," & Val(OrgID) & ",'" & remark2 & "','','" & remarkHindi & "'"
        End If
        If Val(CommAmt) > 0 Then ''Commission Account
            FastQuery = FastQuery & IIf(FastQuery <> "", " UNION ALL SELECT ", " SELECT ") & Val(txtid.Text) & ",'" & CDate(txtEntryDate.Text).ToString("yyyy-MM-dd") & "','" & Me.Text & "','" & Val(10) & "','" & clsFun.ExecScalarStr("Select AccountName from Accounts where Id=10") & "','" & Val(CommAmt) & "','C'," & Val(ServerTag) & "," & Val(OrgID) & ",'" & remark2 & "','','" & remarkHindi & "'"
        End If
        If Val(MandiTax) > 0 Then ''Mandi Account
            FastQuery = FastQuery & IIf(FastQuery <> "", " UNION ALL SELECT ", " SELECT ") & Val(txtid.Text) & ",'" & CDate(txtEntryDate.Text).ToString("yyyy-MM-dd") & "','" & Me.Text & "','" & Val(30) & "','" & clsFun.ExecScalarStr("Select AccountName from Accounts where Id=30") & "','" & Val(MandiTax) & "','C'," & Val(ServerTag) & "," & Val(OrgID) & ",'" & remark2 & "','','" & remarkHindi & "'"
        End If
        If Val(RDFAmt) > 0 Then ''rdf Account
            FastQuery = FastQuery & IIf(FastQuery <> "", " UNION ALL SELECT ", " SELECT ") & Val(txtid.Text) & ",'" & CDate(txtEntryDate.Text).ToString("yyyy-MM-dd") & "','" & Me.Text & "','" & Val(39) & "','" & clsFun.ExecScalarStr("Select AccountName from Accounts where Id=39") & "','" & Val(RDFAmt) & "','C'," & Val(ServerTag) & "," & Val(OrgID) & ",'" & remark2 & "','','" & remarkHindi & "'"
        End If
        If Val(TareAmt) > 0 Then ''Bardana Account
            FastQuery = FastQuery & IIf(FastQuery <> "", " UNION ALL SELECT ", " SELECT ") & Val(txtid.Text) & ",'" & CDate(txtEntryDate.Text).ToString("yyyy-MM-dd") & "','" & Me.Text & "','" & Val(4) & "','" & clsFun.ExecScalarStr("Select AccountName from Accounts where Id=4") & "','" & Val(TareAmt) & "','C'," & Val(ServerTag) & "," & Val(OrgID) & ",'" & remark2 & "','','" & remarkHindi & "'"
        End If
        If Val(Labouramt) > 0 Then ''Labour Account
            FastQuery = FastQuery & IIf(FastQuery <> "", " UNION ALL SELECT ", " SELECT ") & Val(txtid.Text) & ",'" & CDate(txtEntryDate.Text).ToString("yyyy-MM-dd") & "','" & Me.Text & "','" & Val(23) & "','" & clsFun.ExecScalarStr("Select AccountName from Accounts where Id=23") & "','" & Val(Labouramt) & "','C'," & Val(ServerTag) & "," & Val(OrgID) & ",'" & remark2 & "','','" & remarkHindi & "'"
        End If
        If Val(Roff) <> 0 Then ''Account 
            If Val(Roff) < 0 Then
                FastQuery = FastQuery & IIf(FastQuery <> "", " UNION ALL SELECT ", " SELECT ") & Val(txtid.Text) & ",'" & CDate(txtEntryDate.Text).ToString("yyyy-MM-dd") & "','" & Me.Text & "'," & 42 & ",'" & clsFun.ExecScalarStr("Select AccountName from Accounts where Id=42") & "'," & Math.Abs(Val(Roff)) & ",'D' ,'" & Remark & "','" & txtAccount.Text & "','" & remarkHindi & "'"
    ...
    Private Sub ServerCrateLedger()
        ...
        For Each row As DataGridViewRow In dg1.Rows
            With row
                If Val(.Cells(22).Value) > 0 Then ''Party Account
                    FastQuery = FastQuery & IIf(FastQuery <> "", " UNION ALL SELECT ", " SELECT ") & Val(txtid.Text) & "," & Val(clsFun.ExecScalarInt("Select Count(ID) AS NumberOfProducts FROM CrateVoucher Where TransType='Crate Out'") + 1) & ",'" & CDate(txtEntryDate.Text).ToString("yyyy-MM-dd") & "','" & Me.Text & "'," & Val(txtAccountID.Text) & ",'" & txtAccount.Text & "','Crate Out'," & Val(.Cells(21).Value) & ",'" & .Cells(20).Value & "','" & .Cells(22).Value & "', '" & txtVehicleNo.Text & "','','',''," & Val(ServerTag) & "," & Val(OrgID) & ""
                End If
            End With
        Next
        If FastQuery = String.Empty Then Exit Sub
            ...
            With row
                ...
                If .Cells("Charge Name").Value <> "" Then
                    ...
                    If .Cells(3).Value = "+" Then
                        ...
                        FastQuery = FastQuery & IIf(FastQuery <> "", " UNION ALL SELECT ", " SELECT ") & Val(txtid.Text) & ",'" & CDate(txtEntryDate.Text).ToString("yyyy-MM-dd") & "','" & Me.Text & "'," & Val(AcID) & ", '" & AccName & "','" & Val(.Cells(4).Value) & "', 'C','Voucher No:" & txtVoucherNo.Text & "','" & .Cells(4).Value & "','Voucher No:" & txtVoucherNo.Text & "'"
                    Else
                        FastQuery = FastQuery & IIf(FastQuery <> "", " UNION ALL SELECT ", " SELECT ") & Val(txtid.Text) & ",'" & CDate(txtEntryDate.Text).ToString("yyyy-MM-dd") & "','" & Me.Text & "'," & Val(AcID) & ", '" & AccName & "','" & Val(.Cells(4).Value) & "', 'D','Voucher No:" & txtVoucherNo.Text & "','" & .Cells(4).Value & "','Voucher No:" & txtVoucherNo.Text & "'"
                    End If
                End If
            End With
        Next
        ...
        For Each row As DataGridViewRow In Dg2.Rows
            ...
            With row
                ...
                If .Cells("Charge Name").Value <> "" Then
                    ...
                    If .Cells(3).Value = "+" Then
                        '     clsFun.Ledger(0, VchId, SqliteEntryDate, Me.Text, AcID, AccName, .Cells(4).Value, "C", Me.Text & ", Voucher No:" & txtVoucherNo.Text, .Cells(0).Value)
                        FastQuery = FastQuery & IIf(FastQuery <> "", " UNION ALL SELECT ", " SELECT ") & Val(txtid.Text) & ",'" & CDate(txtEntryDate.Text).ToString("yyyy-MM-dd") & "','" & Me.Text & "'," & Val(AcID) & ", '" & AccName & "','" & Val(.Cells(4).Value) & "', 'C'," & Val(ServerTag) & "," & Val(OrgID) & ",'Voucher No:" & txtVoucherNo.Text & "','" & .Cells(4).Value & "','Voucher No:" & txtVoucherNo.Text & "'"
                    Else
                        FastQuery = FastQuery & IIf(FastQuery <> "", " UNION ALL SELECT ", " SELECT ") & Val(txtid.Text) & ",'" & CDate(txtEntryDate.Text).ToString("yyyy-MM-dd") & "','" & Me.Text & "'," & Val(AcID) & ", '" & AccName & "','" & Val(.Cells(4).Value) & "', 'D'," & Val(ServerTag) & "," & Val(OrgID) & ",'Voucher No:" & txtVoucherNo.Text & "','" & .Cells(4).Value & "','Voucher No:" & txtVoucherNo.Text & "'"
                    End If
                End If
            End With
        Next
    ...
    Public Sub FillControls(ByVal id As Integer)
        ...
        Dim sql As String = "Select * from Transaction4 where VoucherID=" & id
        ...
        With dg1
            ...
            For i = 0 To ds.Tables("b").Rows.Count - 1
                ...
                txtVehicleID.Text = Val(ds.Tables("b").Rows(i)("PurchaseID").ToString())
    ...
    Public Sub FillWithNevigation()
        ...
        Dim recordsCount As Integer = clsFun.ExecScalarInt("Select Count(*) FROM Vouchers WHERE transtype = 'Sale (Loose)'  Order By ID ")
        TotalPages = Math.Ceiling(recordsCount / RowCount)
        If clsFun.GetConnection.State = ConnectionState.Open Then clsFun.CloseConnection()
        sSql = "Select * from Vouchers  WHERE transtype = 'Sale (Loose)'   Order By ID LIMIT " + RowCount.ToString() + " OFFSET " + Offset.ToString() + ""
        ...
        End If
        dg1.Rows.Clear()
        Dim sql As String = "Select * from Transaction4 where VoucherID=" & Val(txtid.Text)
        Dim ad1 As New SQLite.SQLiteDataAdapter(sql, clsFun.GetConnection)
        ad1.Fill(ds, "b")
    ...
    Private Sub UpdateRecord()
        ...
        Try
            ...
            UpdateCrate()
            If clsFun.ExecNonQuery("DELETE from Ledger WHERE VourchersID=" & Val(txtid.Text) & ";DELETE from Transaction4 WHERE VoucherID=" & Val(txtid.Text) & "; " &
    ...
    Private Sub delete()
        ...
        If MessageBox.Show("Sure ??", "DELETE", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
            If clsFun.ExecNonQuery("DELETE from Ledger WHERE VourchersID=" & txtid.Text & "; " &
                                   ...
                                   "DELETE from Transaction4 WHERE VoucherID=" & txtid.Text & "; " &
    ...
    Private Sub rowColums()
        dg1.ColumnCount = 28
        dg1.Columns(0).Name = "Item Name" : dg1.Columns(0).Width = 490
        dg1.Columns(1).Name = "Lot No" : dg1.Columns(1).Visible = False
    ...
    Private Sub ItemRowColumns()
        ...
        dgItemSearch.Columns(1).Name = "Item Name" : dgItemSearch.Columns(1).Width = 290
    ...
    Private Sub dgItemSearch_KeyDown(sender As Object, e As KeyEventArgs) Handles dgItemSearch.KeyDown
        If e.KeyCode = Keys.Enter Then
            ...
            txtNug.Focus() : e.SuppressKeyPress = True
        End If

    End Sub
    Private Sub VehicleCoulmns()
        dgVehicle.ColumnCount = 4
        dgVehicle.Columns(0).Name = "LotID" : dgVehicle.Columns(0).Visible = False
        dgVehicle.Columns(1).Name = "Vehicle No." : dgVehicle.Columns(1).Width = 120
        dgVehicle.Columns(2).Name = "Date" : dgVehicle.Columns(2).Width = 100
        dgVehicle.Columns(3).Name = "Bill No" : dgVehicle.Columns(3).Width = 200
    ...
    Private Sub RetriveLot(Optional ByVal condtion As String = "")
        ...
        Dim dt As DataTable
        If BtnSave.Text = "&Save" Then
            sql = "Select VoucherID,VehicleNo,EntryDate,BillNo,(Select ifnull(maX(PurchaseID),0) From Transaction4 Where PurchaseID=PurchaseLoose.VoucherID) As PurchaseID From PurchaseLoose " & _
             " Where PurchaseID=0 and EntryDate <='" & CDate(txtEntryDate.Text).ToString("yyyy-MM-dd") & "' " & condtion & " Group By VoucherID,EntryDate,BillNo"
        Else
            sql = "Select VoucherID,VehicleNo,EntryDate,BillNo,(Select ifnull(maX(PurchaseID),0) From Transaction4 Where PurchaseID=PurchaseLoose.VoucherID) As PurchaseID From PurchaseLoose " & _
             " Where (PurchaseID=" & Val(txtVehicleID.Text) & " or PurchaseID=0) and EntryDate <='" & CDate(txtEntryDate.Text).ToString("yyyy-MM-dd") & "' " & condtion & " Group By VoucherID,EntryDate,BillNo"
        End If
    ...
    Private Sub txtVoucherNo_GotFocus(sender As Object, e As EventArgs) Handles txtVoucherNo.GotFocus, txtVehicleNo.GotFocus, txtEntryDate.GotFocus,
         txtKg.GotFocus, txtRate.GotFocus, txtAccount.GotFocus, txtItem.GotFocus, txtDriverName.GotFocus, txtNug.GotFocus,
        ...
        If txtAccount.Focused = Nothing Then dgCharges.Visible = False : dgVehicle.Visible = False : dgItemSearch.Visible = False
        If txtVehicleNo.Focused = Nothing Then DgAccountSearch.Visible = False : dgVehicle.Visible = False : dgItemSearch.Visible = False
        If txtItem.Focused = Nothing Then DgAccountSearch.Visible = False : dgVehicle.Visible = False : dgCharges.Visible = False
        If txtCharges.Focused = Nothing Then DgAccountSearch.Visible = False : dgVehicle.Visible = False : dgItemSearch.Visible = False
        If txtAccount.Focused = True Then
            If dgVehicle.SelectedRows.Count = 0 Then Exit Sub
            txtVehicleID.Text = Val(dgVehicle.SelectedRows(0).Cells(0).Value)
            txtVehicleNo.Text = dgVehicle.SelectedRows(0).Cells(1).Value
            dgVehicle.Visible = False : AccountRowColumns() : retriveAccounts()
            DgAccountSearch.Visible = True : DgAccountSearch.BringToFront()
        End If
        If txtNug.Focused = True Then
            txtItem.Clear() : txtItemID.Clear()
            txtItemID.Text = Val(dgItemSearch.SelectedRows(0).Cells(0).Value)
            txtItem.Text = dgItemSearch.SelectedRows(0).Cells(1).Value
            itemfill() : dgItemSearch.Visible = False : txtNug.Focus()
            ...
            End If
        End If
        If txtVehicleNo.Focused = True Then
            VehicleCoulmns() : RetriveLot()
            dgVehicle.Visible = True
            dgVehicle.BringToFront()
    ...
    Private Sub txtVoucherNo_LostFocus(sender As Object, e As EventArgs) Handles txtVoucherNo.LostFocus, txtVehicleNo.LostFocus, txtEntryDate.LostFocus,
     txtKg.LostFocus, txtRate.LostFocus, txtAccount.LostFocus, txtItem.LostFocus, txtDriverName.LostFocus, txtNug.LostFocus,
    ...
    Private Sub txtEntryDate_KeyDown(sender As Object, e As KeyEventArgs) Handles txtVoucherNo.KeyDown, txtVehicleNo.KeyDown, txtEntryDate.KeyDown,
           txtKg.KeyDown, txtRate.KeyDown, Cbper.KeyDown, txtAccount.KeyDown, txtItem.KeyDown, txtDriverName.KeyDown,
        ...
        If txtVehicleNo.Focused Then
            If e.KeyCode = Keys.Down Then dgVehicle.Focus() : Exit Sub
        End If
        If DgAccountSearch.Visible = False And dgItemSearch.Visible = False And dgVehicle.Visible = False Then
            If e.KeyCode = Keys.Down Then
                ...
                dg1.Rows(0).Selected = True : dg1.Focus() : Exit Sub
            End If
        End If
    ...
    Private Sub dg1_KeyDown(sender As Object, e As KeyEventArgs) Handles dg1.KeyDown
        If e.KeyCode = Keys.Enter Then
            ...
            txtVehicleID.Text = Val(dg1.SelectedRows(0).Cells(26).Value)
    ...
    Private Sub txtNug_KeyUp(sender As Object, e As EventArgs) Handles txtNug.KeyUp, Cbper.KeyUp,
        txtKg.KeyUp, txtRate.KeyUp, txtBasicAmount.KeyUp, txtchargesAmount.KeyUp,
        txtTotalNetAmount.KeyUp, txtbasicTotal.KeyUp, txtCalculatePer.KeyUp, txtPlusMinus.KeyUp,
       txtOnValue.KeyUp, txttotalCharges.KeyUp, txtTotal.KeyUp, txtComPer.KeyUp, txtMAmt.KeyUp, txtRdfPer.KeyUp, txtTare.KeyUp, txtLabour.KeyUp
    ...
    Private Sub txtTotal_KeyDown(sender As Object, e As KeyEventArgs) Handles txtTotal.KeyDown
        If e.KeyCode = Keys.Enter Then
            ...
            If dg1.SelectedRows.Count = 1 Then
                ...
                dg1.SelectedRows(0).Cells(1).Value = ""
                ...
                dg1.SelectedRows(0).Cells(26).Value = Val(txtVehicleID.Text)
                dg1.SelectedRows(0).Cells(27).Value = Val(lblRoundOff.Text)
                calc() : cleartxt() : dg1.ClearSelection()
            Else
                If lblCrate.Text = "Y" Then
                    dg1.Rows.Add(txtItem.Text, "", Format(Val(txtNug.Text), "0.00"), Format(Val(txtKg.Text), "0.00"), Format(Val(txtRate.Text), "0.00"), Cbper.Text,
                          Format(Val(txtBasicAmount.Text), "0.00"), Format(Val(txtTotal.Text), "0.00"), Val(txtItemID.Text), Format(Val(txtComPer.Text), "0.00"),
                           ...
                           cbCrateMarka.Text, Val(cbCrateMarka.SelectedValue), Val(txtCrateQty.Text), Format(Val(lblTotCharges.Text), "0.00"), Val(cbAccountName.SelectedValue), cbAccountName.Text, Val(txtVehicleID.Text), Val(lblRoundOff.Text))
                Else
                    dg1.Rows.Add(txtItem.Text, "", Format(Val(txtNug.Text), "0.00"), Format(Val(txtKg.Text), "0.00"), Format(Val(txtRate.Text), "0.00"), Cbper.Text,
                          Format(Val(txtBasicAmount.Text), "0.00"), Format(Val(txtTotal.Text), "0.00"), Val(txtItemID.Text), Format(Val(txtComPer.Text), "0.00"),
                           Format(Val(txtComAmt.Text), "0.00"), Format(Val(txtMPer.Text), "0.00"), Format(Val(txtMAmt.Text), "0.00"), Format(Val(txtRdfPer.Text), "0.00"), Format(Val(txtRdfAmt.Text), "0.00"),
                           Format(Val(txtTare.Text), "0.00"), Format(Val(txtTareAmt.Text), "0.00"), Format(Val(txtLabour.Text), "0.00"), Format(Val(txtLaboutAmt.Text), "0.00"), lblCrate.Text,
                           cbCrateMarka.Text, Val(cbCrateMarka.SelectedValue), Val(txtCrateQty.Text), Format(Val(lblTotCharges.Text), "0.00"), Val(0), "", Val(txtVehicleID.Text), Val(lblRoundOff.Text))
                End If
                calc() : cleartxt() : dg1.ClearSelection()
            End If
        End If
                ...
                If .Cells(6).Value <> "" Then
                    ...
                    FastQuery = FastQuery & IIf(FastQuery <> "", " UNION ALL SELECT ", " SELECT ") & "'" & .Cells(1).Value & "','" & .Cells(2).Value & "','" & .Cells(3).Value & "','" & .Cells(4).Value & "'," & _
                             "'" & .Cells(5).Value & "','" & .Cells(6).Value & "','" & .Cells(7).Value & "','" & .Cells(8).Value & "', " & _
                                ...
                                "'" & .Cells(64).Value & "','" & .Cells(65).Value & "','" & .Cells(66).Value & "','" & .Cells(67).Value & "','" & .Cells(68).Value & "','" & .Cells(68).Value & "','" & .Cells(69).Value & "','" & .Cells(71).Value & "'"
                End If
            End With
        Next
        Try
            ...
            sql = "insert into Printing(D1, D2,M1, P1,P2, P3, P4, P5, P6,P7,P8,P9,P10,P11,P12,P13,P14,P15,P16,P17,P18,P19,P20, " & _
                         " P21,P22,P23,P24,P25,P26,P27,P28,P29,P30,P31,P32,P33,P34,P35,P36,P37,P38,P39,P40,P41,P42,P43,P44,P45,P46,P47,P48,P49,P50,P51,P52,P53,P54,P55,P56,P57,P58)" & FastQuery & ""
    ...
    Private Sub btnPrint_Click(sender As Object, e As EventArgs) Handles btnPrint.Click
        pnlWhatsapp.Visible = True : pnlWhatsapp.BringToFront() : txtWhatsappNo.Focus() : radioPrint.Checked = True
    ...
    Private Sub btnPrevious_Click(sender As Object, e As EventArgs) Handles btnPrevious.Click
        If Offset = 0 Then
            Offset = clsFun.ExecScalarInt("Select Count(*) FROM Vouchers WHERE transtype = 'Sale (Loose)'  Order By ID ")
        End If
        Offset -= RowCount
        If Offset <= 0 Then
            Offset = 0
    ...
    Private Sub Button2_Click(sender As Object, e As EventArgs) Handles Button2.Click
        ...
        Else
            Report_Viewer.printReport("\LooseSale.rpt")
        End If
    ...
    Private Sub SendWhatsAppFile(ByVal phoneNumber As String, ByVal message As String, ByVal pdfFilePath As String)
        ...
        End If
    End Sub
    Private Shared Function UploadFile(filePath As String) As String
        Dim url As String = "https://transfer.sh/"
        ...
        Dim hostedFilePath As String = responseString.Trim()
    ...
    Private Sub UsingWhatsappAPI()
        ...
        FilePath = UploadFile(Application.StartupPath & "\Whatsapp\Pdfs\" & GlobalData.PdfName)
    ...
    Private Sub ExpSettings()
        ...
        Dg2.ClearSelection()
    End Sub
    Private Sub txtEntryDate_Leave(sender As Object, e As EventArgs)
        ' ExpSettings()
    End Sub

    Private Sub txtOnValue_TextChanged(sender As Object, e As EventArgs) Handles txtOnValue.TextChanged

    End Sub

    Private Sub dgVehicle_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles dgVehicle.CellContentClick

    End Sub

    Private Sub dgVehicle_KeyDown(sender As Object, e As KeyEventArgs) Handles dgVehicle.KeyDown
        If e.KeyCode = Keys.Enter Then
            If dgVehicle.SelectedRows.Count = 0 Then Exit Sub
            txtVehicleID.Text = Val(dgVehicle.SelectedRows(0).Cells(0).Value)
            txtVehicleNo.Text = dgVehicle.SelectedRows(0).Cells(1).Value
            dgVehicle.Visible = False : txtAccount.Focus()
            e.SuppressKeyPress = True
        End If
        If e.KeyCode = Keys.Back Then txtVehicleNo.Focus()
    End Sub

    Private Sub dgVehicle_MouseDoubleClick(sender As Object, e As MouseEventArgs) Handles dgVehicle.MouseDoubleClick
        If dgVehicle.SelectedRows.Count = 0 Then Exit Sub
        txtVehicleID.Text = Val(dgVehicle.SelectedRows(0).Cells(0).Value)
        txtVehicleNo.Text = dgVehicle.SelectedRows(0).Cells(1).Value
        dgVehicle.Visible = False : txtAccount.Focus()
    ...
    Private Sub dgItemSearch_GotFocus(sender As Object, e As EventArgs) Handles dgItemSearch.GotFocus

    End Sub

    Private Sub txtVehicleNo_KeyUp(sender As Object, e As KeyEventArgs) Handles txtVehicleNo.KeyUp
        If dgVehicle.RowCount = 0 Then Exit Sub
        If txtVehicleNo.Text.Trim() <> "" Then
            RetriveLot(" And upper(VehicleNo) Like upper('%" & txtVehicleNo.Text.Trim() & "%')")
        Else
            RetriveLot()
        End If
        If e.KeyCode = Keys.Escape Then
            If dgVehicle.Visible = True Then dgVehicle.Visible = False
            Exit Sub
        End If
    End Sub
